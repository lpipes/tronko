...
{ // TODO: it would be better to allocate one chunk of memory, but probably it does not matter in practice
	s->name = dupkstring(&ks->name, 1); //name
	s->comment = dupkstring(&ks->comment, 0); //0x0
	s->seq = dupkstring(&ks->seq, 1); //sequence
	s->qual = dupkstring(&ks->qual, 0); // qual if fastq
	s->l_seq = ks->seq.l; // length
}

bseq1_t *bseq_read(int chunk_size, int *n_, void *ks1_, void *ks2_, int number_of_seqs, int paired, int start, int end, int max_readname_length)
{
	kseq_t *ks = (kseq_t*)ks1_, *ks2 = (kseq_t*)ks2_;
	int size = 0, m, n;
	bseq1_t *seqs;
	m = n = 0; seqs = 0;
	//kstring_t names_kstring, read1_kstring, read2_kstring;
	ks = (kseq_t *)calloc(1,sizeof(kseq_t));
	ks2 = (kseq_t *)calloc(1,sizeof(kseq_t));
	...
	for (i = start; i<end; i++){
		...
		if (paired==1){
			ks->name.s = (char*)malloc((strlen(pairedQueryMat->forward_name[i])+1)*sizeof(char));
			strcpy(ks->name.s,pairedQueryMat->forward_name[i]);
			ks->name.l = strlen(pairedQueryMat->forward_name[i]);
			ks->seq.s = (char*)malloc((strlen(pairedQueryMat->query1Mat[i])+1)*sizeof(char));
			strcpy(ks->seq.s,pairedQueryMat->query1Mat[i]);
			ks->seq.l = strlen(pairedQueryMat->query1Mat[i]);
			ks2->name.s = (char*)malloc((strlen(pairedQueryMat->forward_name[i])+1)*sizeof(char));
			strcpy(ks2->name.s,pairedQueryMat->forward_name[i]);
			ks2->name.l = strlen(pairedQueryMat->forward_name[i]);
			ks2->seq.s = (char*)malloc((strlen(pairedQueryMat->query2Mat[i])+1)*sizeof(char));
			strcpy(ks2->seq.s,pairedQueryMat->query2Mat[i]);
			ks2->seq.l = strlen(pairedQueryMat->query2Mat[i]);
		}else{
			ks->name.s = (char*)malloc((strlen(singleQueryMat->name[i])+1)*sizeof(char));
			strcpy(ks->name.s,singleQueryMat->name[i]);
			ks->name.l = strlen(singleQueryMat->name[i]);
			ks->seq.s = (char*)malloc((strlen(singleQueryMat->queryMat[i])+1)*sizeof(char));
			strcpy(ks->seq.s,singleQueryMat->queryMat[i]);
			ks->seq.l = strlen(singleQueryMat->queryMat[i]);
		}
		trim_readno(&ks->name);
		...
		if (size >= chunk_size && (n&1) == 0){
			...
			printf("Reduce Line Number size. Exiting...\n");
			exit(1);
		}
	}
	if (size == 0 && paired != 0) { // test if the 2nd file is finished
