...
/*void getSequenceInRoot(){
        ...
        for(i=0;i<numbase;i++){
                ...
                for(j=0; j<4; j++){
                        ...
                        //printf("minimum is %lf i is %d j is %d PP[%d][i][j] is
%lf\t",minimum,i,j,root,PP[root][i][j]);
                }
                //printf("root is %d\n",root);
                //printf("index is %d\n",index);
                char base;
...
void getSequenceInNode(int rootNum, int node, char *sequenceInNode) {
    ...
    // char *sequenceInNode;
    int index, i, j;
    // if ((sequenceInNode = (char *)malloc(sizeof(char)*(numbaseArr[rootNum])))
...
void getSequenceInNodeWithoutNs(int rootNum, int node, char *sequenceInNode,
                                int *positionsInRoot, int start_position,
                                int end_position) {
    type_of_PP maximum;
    int index, i, j;
    int count = 0;
    ...
    for (i = start_position; i < end_position; i++) {
        ...
        if (index == 0) {
            base = 'A';
            sequenceInNode[count] = base;
            positionsInRoot[count] = i;
            count++;
        } else if (index == 1) {
            base = 'C';
            sequenceInNode[count] = base;
