...
void needleman_wunsch_align2(const char *a, const char *b, size_t len_a,
                             size_t len_b, const scoring_t *scoring,
                             nw_aligner_t *nw, alignment_t *result) {
    aligner_align(nw, a, b, len_a, len_b, scoring, 0);

    // work backwards re-tracing optimal alignment, then shift sequences into
    // place
    ...
    for (; score_x > 0 && score_y > 0; next_char--) {
#ifdef SEQ_ALIGN_VERBOSE
        printf("matrix: %s (%lu,%lu) score: %i\n", MATRIX_NAME(curr_matrix),
               score_x - 1, score_y - 1, curr_score);
#endif
        ...
        if (score_x > 0 && score_y > 0) {
            alignment_reverse_move(&curr_matrix, &curr_score, &score_x,
                                   &score_y, &arr_index, nw);
        }
    }
