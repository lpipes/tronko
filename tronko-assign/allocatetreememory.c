...
void allocatetreememory_for_nucleotide_Arr(int numberOfTrees) {
    ...
    for (i = 0; i < numberOfTrees; i++) {
        for (j = 0; j < (numspecArr[i] * 2 - 1); j++) {
            treeArr[i][j].posteriornc =
                (double **)malloc(numbaseArr[i] * (sizeof(double *)));
            for (k = 0; k < numbaseArr[i]; k++) {
                treeArr[i][j].posteriornc[k] =
...
void allocateTreeArrMemory(int whichPartition, int max_nodename) {
    int i, j, k, l;
    treeArr[whichPartition] =
        malloc((numspecArr[whichPartition] * 2 - 1) * (sizeof(struct node)));
    for (i = 0; i < (numspecArr[whichPartition] * 2 - 1); i++) {
        treeArr[whichPartition][i].up[0] =
            -2; // -2 is uninitialized / NULL value
    ...
    for (i = numspecArr[whichPartition] - 1;
        ...
        treeArr[whichPartition][i].name =
            (char *)malloc(max_nodename * sizeof(char));
...
void allocateMemoryForTaxArr(int whichPartitions, int max_tax_name_len) {
    ...
    for (k = 0; k < whichPartitions; k++) {
        ...
        for (i = 0; i < numspecArr[k]; i++) {
            ...
            taxonomyArr[k][i] =
                (char **)calloc_check(phylogenyLevels, sizeof(char *));
            for (j = 0; j < phylogenyLevels; j++) {
                taxonomyArr[k][i][j] =
                    (char *)calloc_check(max_tax_name_len, sizeof(char));
            }
        }
    }
}

void getReverseComplement(char *read, char *reverseComplement,
    ...
    static const unsigned char basemap[256] = {
        ...
        56,  57,  58,  59,  60,  61,  62,  63,  64,  'T', 'V', 'G', 'H', 'E',
        'F', 'C', 'D', 'I', 'J', 'M', 'L', 'K', 'N', 'O', 'P', 'Q', 'Y', 'S',
        'A', 'A', 'B', 'W', 'X', 'R', 'Z', 91,  92,  93,  94,  95,  64,  't',
        'v', 'g', 'h', 'e', 'f', 'c', 'd', 'i', 'j', 'm', 'l', 'k', 'n', 'o',
        'p', 'q', 'y', 's', 'a', 'a', 'b', 'w', 'x', 'r', 'z', 123, 124, 125,
...
/*void printNewick(FILE* file, int whichRoot, int idx) {
    ...
    fprintf(file,":%.4f", current.bl); // Assuming 4 decimal places for branch
length
}*/
