...
void parse_options(int argc, char **argv, Options *opt) {
    ...
    while (1) {
        ...
        case '5': // print node info
            ...
            if (!success) fprintf(stderr, "Invalid file\n");
            break;
        case 'U':
            opt->unassigned = 1;
            break;
        ...
        case 'f':
            ...
            if (!success) fprintf(stderr, "Invalid reference file.\n");
            break;
        case 'u':
            success = sscanf(optarg, "%lf", &(opt->score_constant));
            if (!success) fprintf(stderr, "Could not read score constant\n");
            break;
        case 't':
        ...
        case 'm':
            ...
            if (!success) fprintf(stderr, "Invalid MSA file\n");
            break;
        case 'd':
        ...
        case 'o':
            ...
            if (!success) fprintf(stderr, "Invalid output results file\n");
            break;
        case 'x':
            success = sscanf(optarg, "%s", opt->taxonomy_file);
            if (!success) fprintf(stderr, "Invalid taxonomy file\n");
            break;
        case 'g':
            success = sscanf(optarg, "%s", opt->read1_file);
            if (!success) fprintf(stderr, "Invalid single read file\n");
            break;
        case '1':
            success = sscanf(optarg, "%s", opt->read1_file);
            if (!success) fprintf(stderr, "Invalid read 1 file.\n");
            break;
        case '2':
            success = sscanf(optarg, "%s", opt->read2_file);
            if (!success) fprintf(stderr, "Invalid read 2 file.\n");
            break;
        case 'a':
            success = sscanf(optarg, "%s", opt->fasta_file);
            if (!success) fprintf(stderr, "Invalid fasta file.\n");
            break;
        case '3':
            success = sscanf(optarg, "%s", opt->print_alignments_dir);
            opt->print_alignments_to_file = 1;
            if (!success) fprintf(stderr, "Invalid directory.\n");
            break;
        case '4':
        ...
        case 'c':
            ...
            if (!success) fprintf(stderr, "Invalid interval\n");
            break;
        case 'e':
