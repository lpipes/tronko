...
void allocateMemForResults(resultsStruct *results, int sizeOfChunk,
                           int num_threads, int numberOfTrees,
                           int print_alignments, int maxNumSpec, int paired,
                           int use_nw, int max_lineTaxonomy,
                           int max_name_length, int max_query_length,
                           int max_numbase, int use_portion, int padding_size,
                           int number_of_total_nodes) {
    int i, j, k;
    if (use_portion == 1) {
        results->positions = malloc(
            (max_query_length + max_query_length + 2 * padding_size + 1) *
            (sizeof(int)));
        results->locQuery = malloc(
            (max_query_length + max_query_length + 2 * padding_size + 1) *
            (sizeof(char)));
    } else {
        results->positions =
            malloc((max_query_length + max_numbase + 1) * (sizeof(int)));
        results->locQuery =
            malloc((max_query_length + max_numbase + 1) * (sizeof(char)));
    }
    results->nodeScores =
        (type_of_PP ***)malloc(MAX_NUM_BWA_MATCHES * (sizeof(type_of_PP **)));
    for (i = 0; i < MAX_NUM_BWA_MATCHES; i++) {
        results->nodeScores[i] =
            (type_of_PP **)malloc(numberOfTrees * (sizeof(type_of_PP *)));
        for (j = 0; j < numberOfTrees; j++) {
            results->nodeScores[i][j] = (type_of_PP *)malloc(
                (2 * numspecArr[j] - 1) * (sizeof(type_of_PP)));
    ...
    if (use_nw == 1) {
        ...
        scoring_init(results->scoring, match, mismatch, gap_open, gap_extend,
...
void freeMemForResults(resultsStruct *results, int sizeOfChunk, int num_threads,
                       int numberOfTrees, int paired, int use_nw,
                       int use_portion, int maxNumSpec,
                       int number_of_total_nodes) {
    int i, j, k;
