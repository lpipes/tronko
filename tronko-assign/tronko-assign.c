            ...
            for (k = 0; k < numbaseArr[i]; k++) {
                for (l = 0; l < 4; l++) {
                    if (treeArr[i][j].posteriornc[k][l] == -1) { // Missing data
                        ...
                        // treeArr[i][j].posteriornc[k][l] = log((c/3) +
                        // ((1-(((4*c)/3) * treeArr[i][j].posteriornc[k][l]))));
                        double d = 1 - c;
                        double e = c / 3;
...
void assignDepthArr(int node0, int node1, int depth, int whichPartitions) {
    if (node0 != -1 && node1 != -1) {
        ...
        assignDepthArr(treeArr[whichPartitions][node0].up[0],
                       treeArr[whichPartitions][node0].up[1], depth + 1,
                       whichPartitions);
        assignDepthArr(treeArr[whichPartitions][node1].up[0],
                       treeArr[whichPartitions][node1].up[1], depth + 1,
                       whichPartitions);
    }
}
...
void printTreeInfo(int whichPartition, int node, FILE *file) {
    int i, j;
    if (treeArr[whichPartition][node].up[0] == -1 &&
        treeArr[whichPartition][node].up[1] == -1) {
        fprintf(file, "%s\t%d\t%d\n", treeArr[whichPartition][node].name,
                whichPartition, node);
        return;
    } else {
        printTreeInfo(whichPartition, treeArr[whichPartition][node].up[0],
                      file);
        printTreeInfo(whichPartition, treeArr[whichPartition][node].up[1],
                      file);
    }
}
...
void run_bwa(int start, int end, bwaMatches *bwa_results, int concordant,
    ...
    if (paired != 0) {
        main_mem(databasefile, end - start, number_of_threads, bwa_results,
                 concordant, numberOfTrees, start, paired, start, end,
                 max_query_length, max_readname_length, max_acc_name);
    } else {
        main_mem(databasefile, end - start, number_of_threads, bwa_results,
                 concordant, numberOfTrees, start, paired, start, end,
                 max_query_length, max_readname_length, max_acc_name);
...
int getKeysCount(int whichRoot, int node, int *minNodes, int matching_nodes,
                 int *ancestors, int numMinNodes) {
    int child0 = treeArr[whichRoot][node].up[0];
    int child1 = treeArr[whichRoot][node].up[1];
    if (child0 != -1 && child1 != -1) {
        matching_nodes += getKeysCount(whichRoot, child0, minNodes,
                                       matching_nodes, ancestors, numMinNodes) +
                          getKeysCount(whichRoot, child1, minNodes,
                                       matching_nodes, ancestors, numMinNodes);
    }
    int i;
...
int LCA_of_nodes(int whichRoot, int root_node, int *minNodes, int numMinNodes) {
    int *ancestors =
        (int *)malloc((2 * numspecArr[whichRoot] - 1) * sizeof(int));
    int i;
    for (i = 0; i < 2 * numspecArr[whichRoot] - 1; i++) {
        ancestors[i] = -1;
    }
    int matching_nodes = 0;
    getKeysCount(whichRoot, root_node, minNodes, matching_nodes, ancestors,
                 numMinNodes);
    int LCA = ancestors[0];
    free(ancestors);
    return LCA;
...
int getLCAofArray_Arr_Multiple(int *voteroot, int whichRoot, int maxNumSpec,
                               int number_of_total_nodes) {
    /*int LCA = minNodes[0];
    int maxDepth = 1000000000;
    int i;
...
void *runAssignmentOnChunk_WithBWA(void *ptr) {
    ...
    /*if (use_nw==0){
            if (use_leaf_portion==1){
                    ...
                    affine_wavefronts =
    affine_wavefronts_new_complete(max_query_length+max_query_length+padding+50+1,max_query_length+max_query_length+padding+50+1,&affine_penalties,NULL,mm_allocator);
            }
    }*/
    char *resultsPath = (char *)malloc(
        (max_readname_length + mstr->max_lineTaxonomy + 120) * sizeof(char));
    bwaMatches *bwa_results =
        (bwaMatches *)malloc((end - (mstr->start)) * sizeof(bwaMatches));
    for (i = 0; i < end - mstr->start; i++) {
        // bwa_results[i].readname =
        // (char*)malloc((max_readname_length+1)*sizeof(char));
        bwa_results[i].concordant_matches_roots =
            (int *)malloc(MAX_NUM_BWA_MATCHES * sizeof(int));
        bwa_results[i].concordant_matches_nodes =
            (int *)malloc(MAX_NUM_BWA_MATCHES * sizeof(int));
        bwa_results[i].discordant_matches_roots =
            (int *)malloc(MAX_NUM_BWA_MATCHES * sizeof(int));
        bwa_results[i].discordant_matches_nodes =
            (int *)malloc(MAX_NUM_BWA_MATCHES * sizeof(int));
        bwa_results[i].use_portion = use_leaf_portion;
        if (use_leaf_portion == 1) {
            bwa_results[i].cigars_forward =
                (char **)malloc(MAX_NUM_BWA_MATCHES * sizeof(char *));
            bwa_results[i].starts_forward =
                (int *)malloc(MAX_NUM_BWA_MATCHES * sizeof(int));
            if (paired == 1) {
                bwa_results[i].cigars_reverse =
                    (char **)malloc(MAX_NUM_BWA_MATCHES * sizeof(char *));
                bwa_results[i].starts_reverse =
                    (int *)malloc(MAX_NUM_BWA_MATCHES * sizeof(int));
            }
        }
        ...
        for (j = 0; j < MAX_NUM_BWA_MATCHES; j++) {
            if (use_leaf_portion == 1) {
                bwa_results[i].cigars_forward[j] =
                    (char *)malloc(MAX_CIGAR * sizeof(char));
                memset(bwa_results[i].cigars_forward[j], '\0', MAX_CIGAR);
                if (paired == 1) {
                    bwa_results[i].cigars_reverse[j] =
                        (char *)malloc(MAX_CIGAR * sizeof(char));
    ...
    int *positionsInRoot;
    if (use_leaf_portion == 1) {
        leaf_sequence = (char *)malloc(
            (max_query_length + max_query_length + 2 * padding + 1) *
            sizeof(char));
        positionsInRoot = (int *)malloc(
            (max_query_length + max_query_length + 2 * padding + 1) *
            sizeof(int));
    } else {
        leaf_sequence = (char *)malloc(
            (max_query_length + mstr->max_numbase + 1) * sizeof(char));
        positionsInRoot = (int *)malloc(
    ...
    for (lineNumber = mstr->start; lineNumber < end; lineNumber++) {
        ...
        int no_add = 0;
        int leaf_iter = 0;
        if (bwa_results[iter].concordant_matches_roots[0] == -1 &&
            mstr->concordant == 1) {
            for (i = 0; i < mstr->ntree; i++) {
                if (bwa_results[iter].discordant_matches_roots[0] < 0) {
                    ...
                    // i=mstr->ntree;
                    break;
                } else if (bwa_results[iter].discordant_matches_roots[i] ==
                           -1) {
                    break;
                } else {
                    results->leaf_coordinates[leaf_iter][0] =
                        bwa_results[iter].discordant_matches_roots[i];
                    results->leaf_coordinates[leaf_iter][1] =
                        bwa_results[iter].discordant_matches_nodes[i];
                    if (use_leaf_portion == 1) {
                        results->starts_forward[leaf_iter] =
                            bwa_results[iter].starts_forward[i];
                        strcpy(results->cigars_forward[leaf_iter],
                               bwa_results[iter].cigars_forward[i]);
                        if (paired == 1) {
                            results->starts_reverse[leaf_iter] =
                                bwa_results[iter].starts_reverse[i];
                            strcpy(results->cigars_reverse[leaf_iter],
                                   bwa_results[iter].cigars_reverse[i]);
                        }
                    }
                }
            ...
            for (i = 0; i < mstr->ntree; i++) {
                if (bwa_results[iter].concordant_matches_roots[i] == -1) {
                    // if
                    // (strlen(bwa_results[iter].concordant_leaf_matches[i])<=3){
                    break;
                } else {
                    results->leaf_coordinates[leaf_iter][0] =
                        bwa_results[iter].concordant_matches_roots[i];
                    results->leaf_coordinates[leaf_iter][1] =
                        bwa_results[iter].concordant_matches_nodes[i];
                    if (use_leaf_portion == 1) {
                        results->starts_forward[leaf_iter] =
                            bwa_results[iter].starts_forward[i];
                        strcpy(results->cigars_forward[leaf_iter],
        ...
        results->minimum[0] = 0;
        if (leaf_iter > 0) {
            // strcmp(results->cigars_forward[0],"*")!=0 &&
            // strcmp(results->cigars_forward[0]," ")!=0){
            if (paired != 0) {
                if (use_nw == 0) {
                    place_paired(
                        pairedQueryMat->query1Mat[lineNumber],
                        pairedQueryMat->query2Mat[lineNumber], rootSeqs,
                        mstr->ntree, results->positions, results->locQuery,
                        results->nodeScores, results->voteRoot, leaf_iter,
                        results->leaf_coordinates, paired, results->minimum,
                        mstr->alignmentsdir,
                        pairedQueryMat->forward_name[lineNumber],
                        pairedQueryMat->reverse_name[lineNumber],
                        print_alignments, leaf_sequence, positionsInRoot,
                        maxNumSpec, results->starts_forward,
                        results->cigars_forward, results->starts_reverse,
                        results->cigars_reverse, print_alignments_to_file,
                        use_leaf_portion, padding, max_query_length,
                        max_numbase, print_all_nodes);
                } else {
                    place_paired_with_nw(
                        pairedQueryMat->query1Mat[lineNumber],
                        pairedQueryMat->query2Mat[lineNumber], rootSeqs,
                        mstr->ntree, results->positions, results->locQuery,
                        results->nw, results->aln, results->scoring,
                        results->nodeScores, results->voteRoot, leaf_iter,
                        results->leaf_coordinates, paired, results->minimum,
                        mstr->alignmentsdir,
                        pairedQueryMat->forward_name[lineNumber],
                ...
                if (use_nw == 0) {
                    place_paired(
                        singleQueryMat->queryMat[lineNumber], NULL, rootSeqs,
                        mstr->ntree, results->positions, results->locQuery,
                        results->nodeScores, results->voteRoot, leaf_iter,
                        results->leaf_coordinates, paired, results->minimum,
                        mstr->alignmentsdir, singleQueryMat->name[lineNumber],
                        NULL, print_alignments, leaf_sequence, positionsInRoot,
                        maxNumSpec, results->starts_forward,
                        results->cigars_forward, results->starts_reverse,
                        results->cigars_reverse, print_alignments_to_file,
                        use_leaf_portion, padding, max_query_length,
        ...
        if (count == 1) {
            ...
            LCA =
                LCA_of_nodes(maxRoot, rootArr[maxRoot], minNodes, numMinNodes);
        } else if (count != 0) {
            for (i = 0; i < count; i++) {
            ...
            for (i = 0; i < count; i++) {
                LCAs[i] = getLCAofArray_Arr_Multiple(
                    results->voteRoot[maxRoots[i]], maxRoots[i], maxNumSpec,
                    number_of_total_nodes);
                if (treeArr[maxRoots[i]][LCAs[i]].taxIndex[1] != -1) {
                    strcpy(
                        LCAnames[i],
                        taxonomyArr[maxRoots[i]]
                                   [treeArr[maxRoots[i]][LCAs[i]].taxIndex[0]]
                                   [treeArr[maxRoots[i]][LCAs[i]].taxIndex[1]]);
            ...
            while (stop == 0 && minLevel <= 6) {
                for (i = 0; i < count; i++) {
                    if (treeArr[maxRoots[i]][LCAs[i]].taxIndex[0] != -1) {
                        for (j = i + 1; j < count; j++) {
                            if (treeArr[maxRoots[j]][LCAs[j]].taxIndex[0] !=
                                -1) {
                                if (strcmp(taxonomyArr
                                               [maxRoots[i]]
                                               [treeArr[maxRoots[i]][LCAs[i]]
                                                    .taxIndex[0]][minLevel],
                                           taxonomyArr
                                               [maxRoots[j]]
                                               [treeArr[maxRoots[j]][LCAs[j]]
                                                    .taxIndex[0]][minLevel]) ==
                                        0 &&
                                    taxonomyArr[maxRoots[i]]
                                               [treeArr[maxRoots[i]][LCAs[i]]
        ...
        for (i = 0; i < max_readname_length + mstr->max_lineTaxonomy + 120;
             i++) {
            resultsPath[i] = '\0';
        }
        int print_un = 1;
        if (count == 1) {
                ...
                for (i = 6; i >= taxIndex1; i--) {
                    if (i == taxIndex1) {
                        strcat(
                            resultsPath,
                            taxonomyArr[maxRoot]
                                       [treeArr[maxRoot][LCA].taxIndex[0]][i]);
                    } else {
                        strcat(
                            resultsPath,
                            taxonomyArr[maxRoot]
            ...
            if (unassigned == 1) {
                ...
                for (i = 6; i >= taxIndex1; i--) {
                    if (i == taxIndex1) {
                        strcat(resultsPath,
                               taxonomyArr[taxRoot][treeArr[taxRoot][taxNode]
                                                        .taxIndex[0]][i]);
                    } else {
                        strcat(resultsPath,
                               taxonomyArr[taxRoot][treeArr[taxRoot][taxNode]
...
int main(int argc, char **argv) {
    ...
    if (opt.fastq == 1 && opt.number_of_lines_to_read % 4 != 0) {
        printf("You chose FASTQ for your queries but the number of lines to "
               "read are not divisible by 4. Change -L to be divisible by 4. "
               "Exiting...\n");
        exit(-1);
    }
    if (opt.fastq == 0 && opt.number_of_lines_to_read % 2 != 0) {
        printf("You chose FASTA for your queries but the number of lines to "
               "read are not divisible by 2. Change -L to be divisible by 2. "
               "Exiting...\n");
        exit(-1);
    }
    ...
    if (opt.print_node_info[0] != '\0') {
        ...
        if (tree_info == NULL) {
            printf("Error opening node info file!\n");
            exit(1);
        }
    ...
    /*if (opt.print_trees_dir[0] = '\0' ){
            ...
            for(i=0; i<numberOfTrees; i++){
                    ...
                    if (NULL==(newick_out=fopen(buffer,"w"))){ puts("Cannot open
    newick file!\n"); exit(-1); } printNewick(newick_out,i,rootArr[i]);
                    fclose(newick_out);
            }
            exit(1);
    }*/
    ...
    if (opt.print_leave_seqs == 1) {
        ...
        if (leaves_file == NULL) {
            printf("Error opening leaves file!\n");
            exit(1);
        }
        for (i = 0; i < numberOfTrees; i++) {
    ...
    if (strcmp("single", opt.paired_or_single) == 0) {
        ...
        if (opt.fastq == 0) {
            singleQueryMat->queryMat =
                (char **)malloc(sizeof(char *) * (numberOfLinesToRead / 2));
            singleQueryMat->name =
                (char **)malloc(sizeof(char *) * (numberOfLinesToRead / 2));
            for (i = 0; i < numberOfLinesToRead / 2; i++) {
                singleQueryMat->queryMat[i] =
                    (char *)malloc(sizeof(char) * (max_query_length + 1));
                singleQueryMat->name[i] =
                    (char *)malloc(sizeof(char) * (max_name_length + 1));
            }
        } else {
            singleQueryMat->queryMat =
                (char **)malloc(sizeof(char *) * (numberOfLinesToRead / 4));
            singleQueryMat->name =
                (char **)malloc(sizeof(char *) * (numberOfLinesToRead / 4));
        ...
        fprintf(results,
                "Readname\tTaxonomic_Path\tScore\tForward_Mismatch\tReverse_"
                "Mismatch\tTree_Number\tNode_Number\n");
        ...
        for (i = 0; i < opt.number_of_cores; i++) {
            ...
            if (opt.fastq == 0) {
                allocateMemForResults(
                    mstr[i].str, numberOfLinesToRead / 2, opt.number_of_cores,
                    numberOfTrees, opt.print_alignments, maxNumSpec, 0,
                    opt.use_nw, max_lineTaxonomy, max_name_length,
                    max_query_length, maxNumBase, opt.use_leaf_portion,
                    opt.padding, number_of_total_nodes);
            } else {
                allocateMemForResults(
                    mstr[i].str, numberOfLinesToRead / 4, opt.number_of_cores,
                    numberOfTrees, opt.print_alignments, maxNumSpec, 0,
                    opt.use_nw, max_lineTaxonomy, max_name_length,
                    max_query_length, maxNumBase, opt.use_leaf_portion,
                    opt.padding, number_of_total_nodes);
        ...
        while (1) {
            if (opt.fastq == 0) {
                returnLineNumber = readInXNumberOfLines(
                    numberOfLinesToRead / 2, seqinfile, 0, opt,
                    max_query_length, max_name_length);
            } else {
                returnLineNumber = readInXNumberOfLines_fastq(
                    numberOfLinesToRead / 4, seqinfile, 0, opt,
                    max_query_length, max_name_length, first_iter);
            }
            ...
            for (i = 0; i < opt.number_of_cores; i++) {
                ...
                j = j + divideFile;
                mstr[i].str->taxonPath =
                    (char **)malloc((end - start) * (sizeof(char *)));
                for (k = 0; k < end - start; k++) {
                    mstr[i].str->taxonPath[k] =
                        malloc((max_name_length + max_lineTaxonomy + 120) *
                               (sizeof(char)));
                }
            }
            for (i = 0; i < opt.number_of_cores; i++) {
        ...
        if (opt.fastq == 0) {
            pairedQueryMat->query1Mat =
                (char **)malloc(sizeof(char *) * numberOfLinesToRead / 2);
            pairedQueryMat->query2Mat =
                (char **)malloc(sizeof(char *) * numberOfLinesToRead / 2);
            pairedQueryMat->forward_name =
                (char **)malloc(sizeof(char *) * numberOfLinesToRead / 2);
            pairedQueryMat->reverse_name =
                (char **)malloc(sizeof(char *) * numberOfLinesToRead / 2);
            for (i = 0; i < numberOfLinesToRead / 2; i++) {
                pairedQueryMat->query1Mat[i] =
                    (char *)malloc(sizeof(char) * max_query_length + 1);
                pairedQueryMat->query2Mat[i] =
                    (char *)malloc(sizeof(char) * max_query_length + 1);
                pairedQueryMat->forward_name[i] =
                    (char *)malloc(sizeof(char) * max_name_length + 1);
                pairedQueryMat->reverse_name[i] =
                    (char *)malloc(sizeof(char) * max_name_length + 1);
        ...
        int returnLineNumber =
            0; //<- this is the number of records that we have read.
        ...
        for (i = 0; i < opt.number_of_cores; i++) {
            ...
            if (opt.fastq == 0) {
                allocateMemForResults(
                    mstr[i].str, numberOfLinesToRead / 2, opt.number_of_cores,
                    numberOfTrees, opt.print_alignments, maxNumSpec, 1,
                    opt.use_nw, max_lineTaxonomy, max_name_length,
                    max_query_length, maxNumBase, opt.use_leaf_portion,
                    opt.padding, number_of_total_nodes);
            } else {
        ...
        while (1) {
            if (opt.fastq == 0) {
                returnLineNumber = readInXNumberOfLines(
                    numberOfLinesToRead / 2, seqinfile_1, 1, opt,
                    max_query_length, max_name_length);
            } else {
                returnLineNumber = readInXNumberOfLines_fastq(
                    numberOfLinesToRead / 4, seqinfile_1, 1, opt,
                    max_query_length, max_name_length, first_iter);
            }
            if (returnLineNumber == 0) break;
            if (opt.fastq == 0) {
                returnLineNumber2 = readInXNumberOfLines(
                    numberOfLinesToRead / 2, seqinfile_2, 2, opt,
                    max_query_length, max_name_length);
            } else {
                returnLineNumber2 = readInXNumberOfLines_fastq(
                    numberOfLinesToRead / 4, seqinfile_2, 2, opt,
                    max_query_length, max_name_length, first_iter);
            }
            ...
            for (i = 0; i < opt.number_of_cores; i++) {
                ...
                if (i == opt.number_of_cores - 1) end = returnLineNumber;
                mstr[i].start = start;
                mstr[i].end = end;
                mstr[i].paired = 1;
                mstr[i].ntree = numberOfTrees;
    ...
    for (i = 0; i < opt.number_of_cores; i++) {
        if (opt.fastq == 0) {
            if (strcmp("single", opt.paired_or_single) == 0) {
                freeMemForResults(mstr[i].str, numberOfLinesToRead / 2,
                                  opt.number_of_cores, numberOfTrees, 0,
                                  opt.use_nw, opt.use_leaf_portion, maxNumSpec,
                                  number_of_total_nodes);
            } else {
                freeMemForResults(mstr[i].str, numberOfLinesToRead / 2,
                                  opt.number_of_cores, numberOfTrees, 1,
                                  opt.use_nw, opt.use_leaf_portion, maxNumSpec,
                                  number_of_total_nodes);
            }
        } else {
            if (strcmp("single", opt.paired_or_single) == 0) {
                freeMemForResults(mstr[i].str, numberOfLinesToRead / 4,
                                  opt.number_of_cores, numberOfTrees, 0,
                                  opt.use_nw, opt.use_leaf_portion, maxNumSpec,
                                  number_of_total_nodes);
            } else {
