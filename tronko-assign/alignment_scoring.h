...
typedef struct {
    ...
    // Turn these on to turn off penalties for gaps at the start/end of
    // alignment
    ...
    // Array of characters that match to everything with the same penalty (i.e.
    // 'N's)
...
#define get_swap_bit(scoring, a, b)                                            \
    bitset32_get((scoring)->swap_set[(size_t)(a)], b)
#define set_swap_bit(scoring, a, b)                                            \
    bitset32_set((scoring)->swap_set[(size_t)(a)], b)
