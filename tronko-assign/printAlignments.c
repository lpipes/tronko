...
void createNewFile(char *referenceName, char *alignments_directory,
                   alignment_t *aln, char *readname) {
    FILE *alignmentFile;
    char fileName[1000];
    snprintf(fileName, 1000, "%s/%s.fasta", alignments_directory,
             referenceName);
...
void printToFile(char *referenceName, char *alignments_directory,
                 alignment_t *aln, char *readname) {
    FILE *alignmentFile;
    char fileName[1000];
    snprintf(fileName, 1000, "%s/%s.fasta", alignments_directory,
...
void createNewFile2(char *referenceName, char *alignments_directory,
                    alignment_t *aln, char *readname, int length_of_read,
                    int *positionsInRoot) {
    FILE *alignmentFile;
    char fileName[1000];
    snprintf(fileName, 1000, "%s/%s.fasta", alignments_directory,
    ...
    while (aln->result_b[counter] != '\0') {
        if (aln->result_b[counter] != '-') {
            if (aln->result_a[counter] != '-') {
                if (iter != 0) {
                    fprintf(alignmentFile, ";%d",
                            positionsInRoot[counter - offset]);
                } else {
                    fprintf(alignmentFile, "%d",
                            positionsInRoot[counter - offset]);
                    iter = 1;
...
void printToFile2(char *referenceName, char *alignments_directory,
                  alignment_t *aln, char *readname, int length_of_read,
                  int *positionsInRoot) {
    FILE *alignmentFile;
    char fileName[1000];
...
void printToFile_WFA(char *referenceName, char *alignments_directory,
                     char *const pattern_alg, char *const text_alg,
                     char *readname, int length_of_read, int *positionsInRoot) {
    FILE *alignmentFile;
    char fileName[1000];
    snprintf(fileName, 1000, "%s", alignments_directory);
    alignmentFile = fopen(fileName, "a");
    if (alignmentFile == NULL) {
        printf("Error No Alignment File!");
        exit(1);
    }
    fprintf(alignmentFile, ">%s\t%s\t%d\n", readname, referenceName,
            length_of_read);
...
void printLeaveSeqsToFile(FILE *output, int node, int whichTree, int numbase) {
    ...
    if (child0 == -1 && child1 == -1) {
        char *leaf_sequence =
            (char *)malloc((numbaseArr[whichTree] + 1) * sizeof(char));
        int i;
