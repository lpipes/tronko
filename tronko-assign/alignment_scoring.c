...
void scoring_init(scoring_t *scoring, int match, int mismatch, int gap_open,
    ...
    if (!no_gaps_in_a || !no_gaps_in_b) {
        scoring->min_penalty =
            MIN3(scoring->min_penalty, gap_open + gap_extend, gap_extend);
        scoring->max_penalty =
            MAX3(scoring->max_penalty, gap_open + gap_extend, gap_extend);
    }
}
...
void scoring_add_mutations(scoring_t *scoring, const char *str,
                           const int *scores, char use_match_mismatch) {
