...
static void alignment_fill_matrices(aligner_t *aligner, char is_sw) {
    ...
    if (is_sw) {
        ...
        for (j = 1, index = score_width; j < score_height;
             j++, index += score_width)
            match_scores[index] = gap_a_scores[index] = gap_b_scores[index] =
                min;
    } else {
    ...
    for (seq_j = 0; seq_j < len_j; seq_j++) {
        for (seq_i = 0; seq_i < len_i; seq_i++) {
            ...
            scoring_lookup(scoring, *(aligner->seq_a + seq_i),
                           *(aligner->seq_b + seq_j), &substitution_penalty,
                           &is_match);
            ...
            if (scoring->no_mismatches && !is_match) {
                ...
                match_scores[index] = MAX4(
                    ...
                    gap_b_scores[index_upleft] + substitution_penalty, min);
            }
            ...
            if (seq_i == len_i - 1 && scoring->no_end_gap_penalty) {
                gap_a_scores[index] =
                    MAX3(match_scores[index_up], gap_a_scores[index_up],
                ...
                gap_a_scores[index] =
                    MAX4(match_scores[index_up] + gap_open_penalty,
                         ...
                         gap_b_scores[index_up] + gap_open_penalty, min);
            } else gap_a_scores[index] = min;

            // Update gap_b_scores[i][j] from position [i-1][j]
            if (seq_j == len_j - 1 && scoring->no_end_gap_penalty) {
                gap_b_scores[index] =
                    MAX3(match_scores[index_left], gap_a_scores[index_left],
                ...
                gap_b_scores[index] =
                    MAX4(match_scores[index_left] + gap_open_penalty,
                         ...
                         gap_b_scores[index_left] + gap_extend_penalty, min);
            } else gap_b_scores[index] = min;
...
void aligner_align(aligner_t *aligner, const char *seq_a, const char *seq_b,
                   size_t len_a, size_t len_b, const scoring_t *scoring,
                   char is_sw) {
...
void alignment_reverse_move(enum Matrix *curr_matrix, score_t *curr_score,
                            size_t *score_x, size_t *score_y, size_t *arr_index,
                            const aligner_t *aligner) {
    ...
    gap_a_open_penalty = gap_b_open_penalty =
        scoring->gap_extend + scoring->gap_open;
    ...
    default:
        fprintf(stderr,
                "Program error: invalid matrix in get_reverse_move()\n");
        fprintf(stderr,
                "Please submit a bug report to: turner.isaac@gmail.com\n");
        exit(EXIT_FAILURE);
    }
    ...
    if ((!scoring->no_gaps_in_a || *score_x == 0 || *score_x == len_i) &&
        ...
        *curr_score = aligner->gap_a_scores[*arr_index];
               ...
               aligner->gap_b_scores[*arr_index] + prev_gap_b_penalty ==
                   *curr_score) {
        *curr_matrix = GAP_B;
        *curr_score = aligner->gap_b_scores[*arr_index];
    } else if (aligner->match_scores[*arr_index] + prev_match_penalty ==
               *curr_score) {
        *curr_matrix = MATCH;
        *curr_score = aligner->match_scores[*arr_index];
        ...
        fprintf(stderr, "Program error: traceback fail (get_reverse_move)\n"
                        "This may be due to an integer overflow if your "
                        "sequences are long or scores\n"
                        "are large. If this is the case using smaller scores "
                        "or shorter sequences may\n"
...
void alignment_print_matrices(const aligner_t *aligner) {
    ...
    for (j = 0; j < aligner->score_height; j++) {
        ...
        for (i = 0; i < aligner->score_width; i++) {
            printf("\t%3i",
                   (int)ARR_LOOKUP(match_scores, aligner->score_width, i, j));
        }
        putc('\n', stdout);
    }
    printf("gap_a_scores:\n");
    for (j = 0; j < aligner->score_height; j++) {
        ...
        for (i = 0; i < aligner->score_width; i++) {
            printf("\t%3i",
                   (int)ARR_LOOKUP(gap_a_scores, aligner->score_width, i, j));
        }
        putc('\n', stdout);
    }
    printf("gap_b_scores:\n");
    for (j = 0; j < aligner->score_height; j++) {
        ...
        for (i = 0; i < aligner->score_width; i++) {
            printf("\t%3i",
                   (int)ARR_LOOKUP(gap_b_scores, aligner->score_width, i, j));
        }
        putc('\n', stdout);
    }
