...
int readInXNumberOfLines_fastq(int numberOfLinesToRead, gzFile query_reads,
    ...
    while (gzgets(query_reads, buffer, buffer_size) != NULL) {
        ...
        if (buffer[0] == '@' && whichPair == 1) {
            ...
            seqname[i - 1] = '\0';
            memset(pairedQueryMat->forward_name[iter], '\0',
                   max_readname_length);
            ...
            for (i = 0; i < size - 1; i++) {
                if (pairedQueryMat->forward_name[iter][i] == '1' &&
                    pairedQueryMat->forward_name[iter][i - 1] == '_') {
                    tempname[i] = '2';
                } else {
            ...
            /*while(strcmp(tempname,seqname)!=0){
                    for(i=0; i<size-1; i++){
                            if ( pairedQueryMat->forward_name[skipped][i] == '1'
            && pairedQueryMat->forward_name[skipped][i-1] == '_'){ tempname[i] =
            '2'; }else{ tempname[i] = seqname[i];
                            }
                    }
                    tempname[size-1]='\0';
                    skipped++;
            ...
            if (skipped == iter) {
                memset(pairedQueryMat->reverse_name[iter], '\0',
                       max_readname_length);
...
// this functions returns the number of 'reads'. This is half the number of
// actual lines. if eof it returns zero.
int readInXNumberOfLines(int numberOfLinesToRead, gzFile query_reads,
                         int whichPair, Options opt, int max_query_length,
    ...
    while (gzgets(query_reads, buffer, buffer_size) != NULL) {
        ...
        if (first_iter == 1) {
            if (buffer[0] != '>') {
                printf("Query reads are not in FASTA format. Try specifying -q "
                       "if using FASTQ reads.\n");
                exit(-1);
            }
            first_iter = 0;
        }
...
void shiftUp(int iter, int jump, int numberOfLinesToRead) {
    ...
    for (i = iter; i < numberOfLinesToRead - jump; i++) {
        strcpy(pairedQueryMat->forward_name[i],
               pairedQueryMat->forward_name[i + jump]);
        strcpy(pairedQueryMat->query1Mat[i],
               pairedQueryMat->query1Mat[i + jump]);
    }
    ...
    while (gzgets(file, buffer, FASTA_MAXLINE) != NULL) {
        ...
        if (s == NULL || s[0] == '\0') {
            if (buffer[0] == '>' || buffer[0] == '@') {
                fprintf(stderr,
                        "Fatal: encountered an empty header at record for read "
                        "\"%s\" — aborting.\n",
                        last_name[0] ? last_name : "<unknown>");
            } else {
                fprintf(stderr,
                        "Fatal: encountered an empty sequence for read \"%s\" "
                        "— aborting.\n",
                        last_name[0] ? last_name : "<unknown>");
            }
            free(buffer);
