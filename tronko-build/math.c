...
double LnGamma(double alpha) {
    /* returns ln(gamma(alpha)) for alpha>0, accurate to 10 decimal places.
       ...
       function. Communications of the Association for Computing Machinery,
       9:684
       */
...
double PointNormal(double prob) {
    /* returns z so that Prob{x<z}=prob where x ~ N(0,1) and
       ...
       (1974) The percentage points of the normal distribution. Applied
       Statistics 22: 96-97 (AS70)
...
void definegammaquantiles(int k, double par[2]) {
    ...
    for (i = 0; i < k; i++) {
        statevector[i] =
            PointGamma(((double)i + 0.5) / (double)k, par[0], par[1]);
        // mean+=statevector[i];
    }
}
...
/*A is the matrix, rr = root real(nx1), ri = root imaginary(nx1), vr = real part
 * of eigenvector(nxn), w = working space (size of 2n), set job to 1 (decides if
 * both eigenvectors and eigenvalues should be calculated*/
    ...
    while (en >= low) {
        ...
        for (;;) {
                ...
                if (q < 0) { /* complex pair */
                    ...
                    if (job) {
                        ...
                        for (j = en - 1; j < n; j++) {
                            z = mat[pos(en - 1, j, n)];
                            mat[pos(en - 1, j, n)] =
                                q * z + p * mat[pos(en, j, n)];
                            mat[pos(en, j, n)] = q * mat[pos(en, j, n)] - p * z;
                        }
                        for (i = 0; i <= en; i++) {
                            z = mat[pos(i, en - 1, n)];
                            mat[pos(i, en - 1, n)] =
                                q * z + p * mat[pos(i, en, n)];
            ...
            if (niter != 0 && niter % 10 == 0) {
                ...
                s = fabs(mat[pos(en, en - 1, n)]) +
                    fabs(mat[pos(en - 1, en - 2, n)]);
                x = y = 0.75 * s;
                w = -0.4375 * s * s;
            }
            niter++;
            ...
            for (m = en - 2; m >= l; m--) {
                ...
                r /= s;
                if (m == l ||
                    fabs(mat[pos(m, m - 1, n)]) * (fabs(q) + fabs(r)) <=
                        eps *
    ...
    if (norm != 0) {
        ...
        for (en = n - 1; en >= 0; en--) {
            ...
            if ((q = vali[en]) < 0) { /* complex vector */
                m = en - 1;
                if (fabs(mat[pos(en, en - 1, n)]) >
                    fabs(mat[pos(en - 1, en, n)])) {
                ...
                for (i = en - 2; i >= 0; i--) {
                    ...
                    if (vali[i] < 0) {
                        ...
                        if (vali[i] == 0) {
                            ...
                            x = mat[pos(i, i + 1, n)];
                            y = mat[pos(i + 1, i, n)];
                            v.re = (valr[i] - p) * (valr[i] - p) +
                                   vali[i] * vali[i] - q * q;
                            v.im = (valr[i] - p) * 2 * q;
                            if ((fabs(v.re) + fabs(v.im)) == 0) {
                                v.re = eps * norm *
                                       (fabs(w) + fabs(q) + fabs(x) + fabs(y) +
                                        fabs(z));
                            }
                            v = cdiv(compl(x * r - z * ra + q * sa,
                                           x * s - z * sa - q * ra),
                                     v);
                            mat[pos(i, en - 1, n)] = v.re;
                            mat[pos(i, en, n)] = v.im;
...
{
    int i, j;
    double RIVAL[STATESPACE], RIVEC[STATESPACE][STATESPACE],
        A[STATESPACE][STATESPACE], workspace[2 * STATESPACE];

    for (i = 0; i < STATESPACE; i++)
        for (j = 0; j < STATESPACE; j++) A[i][j] = 0.0;
    A[0][0] = -lambda;
    A[0][1] = lambda;
    ...
    if (eigen(1, A[0], STATESPACE, RRVAL, RIVAL, RRVEC[0], RIVEC[0],
              workspace) != 0) {
    ...
    for (i = 0; i < STATESPACE; i++)
        for (j = 0; j < STATESPACE; j++) LRVEC[i][j] = RRVEC[i][j];
...
void maketransitionmatrix(int matnum, double t) {
    ...
    for (k = 0; k < STATESPACE; k++) EXPOS[k] = exp(t * RRVAL[k]);
    for (i = 0; i < STATESPACE; i++) {
        for (j = 0; j < STATESPACE; j++) {
            ...
            for (k = 0; k < STATESPACE; k++) {
                if (matnum == 0)
                    PMAT1[i][j] =
                        PMAT1[i][j] + RRVEC[k][j] * LRVEC[i][k] * EXPOS[k];
                else
                    PMAT2[i][j] =
                        PMAT2[i][j] + RRVEC[k][j] * LRVEC[i][k] * EXPOS[k];
            }
        }
    }
    /*    if (matnum==1){
...
double pdata(double theta, int s, int nt, int nb) {
    ...
    for (k = 1; k < n; k++) {
        p = exp(Logfactorial[n - 2] - Logfactorial[k - 1] -
                Logfactorial[n - 2 - k + 1] +
                (double)(s + 1) * log(theta / ((double)k + theta)));
