...
double LRVEC[STATESPACE][STATESPACE], RRVEC[STATESPACE][STATESPACE],
    RRVAL[STATESPACE], PMAT1[STATESPACE][STATESPACE],
    PMAT2[STATESPACE][STATESPACE];
...
/*This function calculates the number of  desdencents of each node in the tree
 * stored in tree[node].nd*/
int get_number_descendantsArr(int node, struct masterArr *m) {
    if (m->tree[node].up[0] == -1) return (m->tree[node].nd = 1);
    else
        return (m->tree[node].nd =
                    (get_number_descendantsArr(m->tree[node].up[0], m) +
                     get_number_descendantsArr(m->tree[node].up[1], m)));
}
...
void changePP_parents_Arr(int node, int whichRoot) {
    ...
    for (i = 0; i < numbaseArr[whichRoot]; i++) {
        for (j = 0; j < 4; j++) {
            if (treeArr[whichRoot][child0].posteriornc[i][j] == -1 &&
                treeArr[whichRoot][child1].posteriornc[i][j] == -1) {
                treeArr[whichRoot][node].posteriornc[i][j] = -1;
            }
        }
    }
...
void assignDepthArr(int node0, int node1, int depth, struct masterArr *m) {
    if (node0 != -1 && node1 != -1) {
        ...
        assignDepthArr(m->tree[node0].up[0], m->tree[node0].up[1], depth + 1,
                       m);
        assignDepthArr(m->tree[node1].up[0], m->tree[node1].up[1], depth + 1,
                       m);
    }
}
...
void assignTaxonomyToLeavesArr(char *tax, struct masterArr *m, int max_nodename,
                               int max_tax_name) {
    ...
    for (i = 0; i < 2 * m->numspec - 1; i++) {
        if (i < m->numspec - 1) {
            ...
            if ((taxonomy_file = fopen(tax, "r")) == (FILE *)NULL)
                printf("*** taxonomy file could not be opened.\n");
            while (fgets(buffer, BUFFER_SIZE, taxonomy_file) != NULL) {
                ...
                if (strcmp(accessionID, m->tree[i].name) == 0) {
                    ...
                    while (j > -1) {
                        taxLevelName =
                            strtok_r(lineTaxonomy, ";", &lineTaxonomy);
                        strncpy(m->taxonomy[i - m->numspec + 1][j],
                                taxLevelName, max_tax_name);
                        j--;
                    }
                }
            }
    ...
    /*if ( child0 == -1 && child1 == -1 ){
            if (( taxonomy_file = fopen(tax,"r")) == (FILE *) NULL) printf("***
    taxonomy file could not be opened.\n"); while(
    fgets(buffer,BUFFER_SIZE,taxonomy_file) != NULL){ lineAccession =
    strtok(buffer,"\t"); lineTaxonomy = strtok(NULL,"\n");
                    assert(strlen(lineAccession) <= max_nodename);
                    strncpy(&accessionID[0], lineAccession, max_nodename);
                    if ( strcmp(accessionID,m->tree[node].name)==0 ){
                            ...
                            while(j>-1){
                                    taxLevelName =
    strtok_r(lineTaxonomy,";",&lineTaxonomy);
                                    strncpy(m->taxonomy[node-m->numspec+1][j],
    taxLevelName, max_tax_name); j--;
                            }
                    }
            }
            fclose(taxonomy_file);
    ...
    if (m->tree[node].up[0] != -1) {
        if (m->tree[node].taxIndex[0] == -1) {
            ...
            if (taxIndexA[0] == -1 || taxIndexB[0] == -1) {
                ...
                int maxABLevel =
                    (taxIndexA[1] > taxIndexB[1]) ? taxIndexA[1] : taxIndexB[1];
                // int maxABLevel = 0;
                for (i = maxABLevel; i < 7; i++) {
                    if (strcmp(m->taxonomy[taxIndexA[0]][i],
                               m->taxonomy[taxIndexB[0]][i]) == 0) {
                        phylogenyLevel = i;
...
int *getTaxonomyArr_UsePartitions(int node, int whichPartitions,
                                  char ****taxonomyArr_heap) {
    int *taxIndexA = NULL;
    int *taxIndexB = NULL;
    int *taxonomyOfNode = malloc(sizeof(int) * 2);
    taxonomyOfNode[0] = -1;
    ...
    if (treeArr[whichPartitions][node].up[0] != -1) {
        if (treeArr[whichPartitions][node].taxIndex[0] == -1) {
            taxIndexA = getTaxonomyArr_UsePartitions(
                treeArr[whichPartitions][node].up[0], whichPartitions,
                taxonomyArr_heap);
            taxIndexB = getTaxonomyArr_UsePartitions(
                treeArr[whichPartitions][node].up[1], whichPartitions,
                taxonomyArr_heap);
            if (taxIndexA[0] == -1 || taxIndexB[0] == -1) {
                ...
                for (i = maxABLevel; i < 7; i++) {
                    if (strcmp(
                            taxonomyArr_heap[whichPartitions][taxIndexA[0]][i],
                            taxonomyArr_heap[whichPartitions][taxIndexB[0]]
                                            [i]) == 0) {
                        phylogenyLevel = i;
                        taxonomyOfNode[0] = taxIndexA[0];
                        taxonomyOfNode[1] = phylogenyLevel;
                        treeArr[whichPartitions][node].taxIndex[0] =
                            taxIndexA[0];
                        treeArr[whichPartitions][node].taxIndex[1] =
                            phylogenyLevel;
                        break;
                    }
                }
        ...
        if (buffer[0] == '>') {
            ...
            if (firstIter == 1) {
                ...
                for (i = 0; i < master->numspec; i++) {
                    master->msa[i] =
                        (int *)malloc(master->numbase * (sizeof(int)));
                }
                firstIter = 0;
                i = 0;
            }
            for (m = 0; m < master->numbase; m++) {
                ...
                if ((c != ' ') && (c != '\t')) {
                    if (c == 'a' || c == 'A') master->msa[row][m] = 0;
                        ...
                        printf("\nBAD BASE (%c) in species %i base %i", c,
                               row + 1, m + 1);
                        scanf("%i", &row);
                        exit(-1);
                    }
...
char *readNewickFile(FILE *file) {
    ...
    if (buffer == NULL) {
        fprintf(stderr,
                "Error: Memory allocation failed while reading Newick file.\n");
        exit(EXIT_FAILURE);
    }
...
int *findLeavesOfMinVarArr(int node, int *leafNodeList, int size,
                           struct masterArr *m) {
    int child0 = m->tree[node].up[0];
    int child1 = m->tree[node].up[1];
    int i = 0;
...
char *removeDashArr(int *sequenceWithDash, int length,
                    char *sequenceWithoutDash) {
    int i, j;
    int countDash;
    countDash = 0;
    for (i = 0; i < length; i++) {
        if (sequenceWithDash[i] < 4) {
            if (sequenceWithDash[i] == 0) {
                sequenceWithoutDash[countDash] = 'A';
            } else if (sequenceWithDash[i] == 1) {
                sequenceWithoutDash[countDash] = 'C';
            } else if (sequenceWithDash[i] == 2) {
                sequenceWithoutDash[countDash] = 'G';
            } else {
                sequenceWithoutDash[countDash] = 'T';
            }
            countDash++;
        }
    }
...
void printPartitionsToFileArr(int *partition1, int partition1size,
                              int *partition2, int partition2size,
                              int *partition3, int partition3size,
                              int whichPartitions, int partitionCount,
                              Options opt, struct masterArr *m) {
    int i = 0;
    int j = 0;
    ...
    if (opt.prefix[0] == '\0') {
        snprintf(buf_fasta, BUFFER_SIZE, "%s/partition%d.fasta",
                 opt.partitions_directory, which);
    } else {
        snprintf(buf_fasta, BUFFER_SIZE, "%s/%spartition%d.fasta",
                 opt.partitions_directory, opt.prefix, which);
    }
    FILE *p1 = fopen(buf_fasta, "w");
    if (opt.prefix[0] == '\0') {
        snprintf(buf_tax, BUFFER_SIZE, "%s/partition%d_taxonomy.txt",
                 opt.partitions_directory, which);
    } else {
        snprintf(buf_tax, BUFFER_SIZE, "%s/%spartition%d_taxonomy.txt",
                 opt.partitions_directory, opt.prefix, which);
    }
    ...
    for (i = 0; i < partition1size; i++) {
        ...
        seqWithoutDash = removeDashArr(m->msa[partition1[i] - m->numspec + 1],
                                       m->numbase, seqWithoutDash);
        fprintf(p1, "%s\n", seqWithoutDash);
        int taxnode = 0;
        fprintf(p1_tax, "%s\t%s;%s;%s;%s;%s;%s;%s\n",
                m->names[partition1[i] - m->numspec + 1],
                m->taxonomy[m->tree[partition1[i]].taxIndex[0]][6],
                m->taxonomy[m->tree[partition1[i]].taxIndex[0]][5],
                m->taxonomy[m->tree[partition1[i]].taxIndex[0]][4],
                m->taxonomy[m->tree[partition1[i]].taxIndex[0]][3],
                m->taxonomy[m->tree[partition1[i]].taxIndex[0]][2],
                m->taxonomy[m->tree[partition1[i]].taxIndex[0]][1],
                m->taxonomy[m->tree[partition1[i]].taxIndex[0]][0]);
    ...
    for (i = 0; i < partition2size; i++) {
        ...
        seqWithoutDash = removeDashArr(m->msa[partition2[i] - m->numspec + 1],
                                       m->numbase, seqWithoutDash);
        fprintf(p2, "%s\n", seqWithoutDash);
        fprintf(p2_tax, "%s\t%s;%s;%s;%s;%s;%s;%s\n",
                m->names[partition2[i] - m->numspec + 1],
                m->taxonomy[m->tree[partition2[i]].taxIndex[0]][6],
                m->taxonomy[m->tree[partition2[i]].taxIndex[0]][5],
                m->taxonomy[m->tree[partition2[i]].taxIndex[0]][4],
                m->taxonomy[m->tree[partition2[i]].taxIndex[0]][3],
                m->taxonomy[m->tree[partition2[i]].taxIndex[0]][2],
                m->taxonomy[m->tree[partition2[i]].taxIndex[0]][1],
                m->taxonomy[m->tree[partition2[i]].taxIndex[0]][0]);
    ...
    for (i = 0; i < partition3size; i++) {
        ...
        seqWithoutDash = removeDashArr(m->msa[partition3[i] - m->numspec + 1],
                                       m->numbase, seqWithoutDash);
        fprintf(p3, "%s\n", seqWithoutDash);
        fprintf(p3_tax, "%s\t%s;%s;%s;%s;%s;%s;%s\n",
                m->names[partition3[i] - m->numspec + 1],
                m->taxonomy[m->tree[partition3[i]].taxIndex[0]][6],
                m->taxonomy[m->tree[partition3[i]].taxIndex[0]][5],
                m->taxonomy[m->tree[partition3[i]].taxIndex[0]][4],
                m->taxonomy[m->tree[partition3[i]].taxIndex[0]][3],
                m->taxonomy[m->tree[partition3[i]].taxIndex[0]][2],
                m->taxonomy[m->tree[partition3[i]].taxIndex[0]][1],
                m->taxonomy[m->tree[partition3[i]].taxIndex[0]][0]);
        for (j = 0; j < m->numbase; j++) {
            seqWithoutDash[j] = '\0';
        }
    }
    ...
    for (i = 0; i < m->numbase; i++) {
        for (j = 0; j < m->numspec; j++) {
            for (k = j + 1; k < m->numspec; k++) {
                if (m->msa[partition[j] - m->numspec + 1][i] ==
                    m->msa[partition[k] - m->numspec + 1][i]) {
                    SPscore = SPscore + 3;
                } else if (m->msa[partition[j] - m->numspec + 1][i] != 4 &&
                           m->msa[partition[k] - m->numspec + 1][i] != 4 &&
                           m->msa[partition[j] - m->numspec + 1][i] !=
                               m->msa[partition[k] - m->numspec + 1][i]) {
                    SPscore = SPscore - 2;
                } else {
                    SPscore--;
...
void findMinVarianceArr(int node, int size, struct masterArr *m) {
    ...
    double variance =
        (double)((num_ancestors - mean) * (num_ancestors - mean) +
                 (num_children0 - mean) * (num_children0 - mean) +
                 (num_children1 - mean) * (num_children1 - mean)) /
        3;
...
void createNewRoots(int rootCount, Options opt, int max_nodename,
                    int max_lineTaxonomy, struct masterArr *m) {
    int i, j, k, count;
    char buffer[BUFFER_SIZE];
    int *leaves;
    gzFile partition;
    ...
    int *partition1 =
        (int *)malloc(sizeof(int) * m->tree[m->tree[minVarNode].up[0]].nd);
    for (i = 0; i < m->tree[m->tree[minVarNode].up[0]].nd; i++) {
        partition1[i] = -1;
    }
    int *partition2 =
        (int *)malloc(sizeof(int) * m->tree[m->tree[minVarNode].up[1]].nd);
    for (i = 0; i < m->tree[m->tree[minVarNode].up[1]].nd; i++) {
        partition2[i] = -1;
    }
    int *partition3 = (int *)malloc(
        sizeof(int) * (m->tree[m->root].nd - m->tree[minVarNode].nd));
    for (i = 0; i < (m->tree[m->root].nd - m->tree[minVarNode].nd); i++) {
        partition3[i] = -1;
    }
    partition1 =
        findLeavesOfMinVarArr(m->tree[minVarNode].up[0], partition1,
                              m->tree[m->tree[minVarNode].up[0]].nd, m);
    partition2 =
        findLeavesOfMinVarArr(m->tree[minVarNode].up[1], partition2,
                              m->tree[m->tree[minVarNode].up[1]].nd, m);
    partition3 = findLeavesOfMinVarArr(
        m->root, partition3, (m->tree[m->root].nd - m->tree[minVarNode].nd), m);
    ...
    if (partitionSizes[0] < 4 || partitionSizes[1] < 4 ||
        partitionSizes[2] < 4) {
    ...
    if (partitionCount > opt.restart) {
        printPartitionsToFileArr(
            partition1, partitionSizes[0], partition2, partitionSizes[1],
            partition3, partitionSizes[2], rootCount, partitionCount, opt, m);
    }
    ...
    for (which = rootCount; which < rootCount + 3; which++) {
        if (rootCount > opt.restart) {
            ...
            if (opt.prefix[0] == '\0') {
                snprintf(buf2, BUFFER_SIZE, "%s/partition%d.fasta",
                         opt.partitions_directory, which);
                snprintf(buf3, BUFFER_SIZE, "%s/partition%d_MSA.fasta",
                         opt.partitions_directory, which);
            } else {
                snprintf(buf2, BUFFER_SIZE, "%s/%spartition%d.fasta",
                         opt.partitions_directory, opt.prefix, which);
                snprintf(buf3, BUFFER_SIZE, "%s/%spartition%d_MSA.fasta",
                         opt.partitions_directory, opt.prefix, which);
            }
            ...
            if (pid == -1) {
                ...
                char *arguments[] = {"famsa", dasht, famsa_threads,
                                     buf2,    buf3,  NULL};
            ...
            if (opt.prefix[0] == '\0') {
                snprintf(buf, BUFFER_SIZE,
                         "sed -i ':a; $!N; /^>/!s/\\n\\([^>]\\)/\\1/; ta; P; "
                         "D' %s/partition%d_MSA.fasta",
                         opt.partitions_directory, which);
            } else {
                snprintf(buf, BUFFER_SIZE,
                         "sed -i ':a; $!N; /^>/!s/\\n\\([^>]\\)/\\1/; ta; P; "
                         "D' %s/%spartition%d_MSA.fasta",
                         opt.partitions_directory, opt.prefix, which);
            }
            status = system(buf);
            if (opt.prefix[0] == '\0') {
                snprintf(buf, BUFFER_SIZE,
                         "fasta2phyml.pl %s/partition%d_MSA.fasta",
                         opt.partitions_directory, which);
            } else {
                snprintf(buf, BUFFER_SIZE,
                         "fasta2phyml.pl %s/%spartition%d_MSA.fasta",
                         opt.partitions_directory, opt.prefix, which);
            }
            status = system(buf);
            if (opt.prefix[0] == '\0') {
                if (opt.fasttree == 0) {
                    snprintf(buf, BUFFER_SIZE,
                             "raxmlHPC-PTHREADS --silent -m GTRGAMMA -w %s/ -n "
                             "partition%d -p 1234 -T 8 -s "
                             "%s/partition%d_MSA.phymlAln",
                             opt.partitions_directory, which,
                             opt.partitions_directory, which);
                } else {
                    snprintf(
                        buf, BUFFER_SIZE,
                        "FastTree -gtr -gamma -nt %s/partition%d_MSA.fasta > "
                        "%s/RAxML_bestTree.partition%d.reroot",
                        opt.partitions_directory, which,
                        opt.partitions_directory, which);
                }
            } else {
                if (opt.fasttree == 0) {
                    snprintf(buf, BUFFER_SIZE,
                             "raxmlHPC-PTHREADS --silent -m GTRGAMMA -w %s/ -n "
                             "%spartition%d -p 1234 -T 8 -s "
                             "%s/%spartition%d_MSA.phymlAln",
                             opt.partitions_directory, opt.prefix, which,
                             opt.partitions_directory, opt.prefix, which);
                } else {
                    snprintf(
            ...
            if (opt.prefix[0] == '\0') {
                snprintf(buf4, BUFFER_SIZE, "%s/RAxML_bestTree.partition%d",
                         opt.partitions_directory, which);
                snprintf(buf5, BUFFER_SIZE,
                         "%s/RAxML_bestTree.partition%d.reroot",
                         opt.partitions_directory, which);
            } else {
                snprintf(buf4, BUFFER_SIZE, "%s/RAxML_bestTree.%spartition%d",
                         opt.partitions_directory, opt.prefix, which);
                snprintf(buf5, BUFFER_SIZE,
                         "%s/RAxML_bestTree.%spartition%d.reroot",
                         opt.partitions_directory, opt.prefix, which);
            }
            pid = 0;
            // int pipefd[2];
            ...
            if (pid == -1) {
                ...
                // int reroot_file;
                // reroot_file = open(buf5, O_WRONLY | O_TRUNC | O_CREAT,
                // S_IRUSR | S_IRGRP | S_IWGRP | S_IWUSR); dup2(reroot_file,1);
                // close(reroot_file);
                /*char *arguments[3];
        ...
        if (opt.prefix[0] == '\0') {
            snprintf(buf, BUFFER_SIZE, "%s/partition%d_MSA.fasta",
                     opt.partitions_directory, which);
        } else {
            snprintf(buf, BUFFER_SIZE, "%s/%spartition%d_MSA.fasta",
                     opt.partitions_directory, opt.prefix, which);
        }
        printf("buffer: %s\n", buf);
        ...
        gzclose(partition);
        t->tree =
            (struct node *)malloc((2 * t->numspec - 1) * sizeof(struct node));
        ...
        gzclose(partition);
        if (opt.prefix[0] == '\0') {
            snprintf(buf, BUFFER_SIZE, "%s/RAxML_bestTree.partition%d.reroot",
                     opt.partitions_directory, which);
        } else {
            snprintf(buf, BUFFER_SIZE, "%s/RAxML_bestTree.%spartition%d.reroot",
                     opt.partitions_directory, opt.prefix, which);
        }
        strcpy(t->filename, buf);
        if (opt.fasttree == 0) {
            allocateTreeArrMemory(t, max_nodename);
            if (NULL == (partitionTree = fopen(buf, "r"))) {
                puts("Cannot open partition tree file!");
                exit(-1);
            }
            comma = 0;
            tip = 0;
        ...
        if (opt.prefix[0] == '\0') {
            snprintf(buf, BUFFER_SIZE, "%s/partition%d_taxonomy.txt",
                     opt.partitions_directory, which);
        } else {
            snprintf(buf, BUFFER_SIZE, "%s/%spartition%d_taxonomy.txt",
                     opt.partitions_directory, opt.prefix, which);
        }
        printf("buffer: %s\n", buf);
        for (j = 0; j < t->numspec; j++) {
            ...
            for (k = 0; k < 7; k++) {
                t->taxonomy[j][k] =
                    (char *)calloc_check(max_lineTaxonomy, sizeof(char));
            }
        }
        ...
        if (opt.use_spscore == 1 && opt.use_min_leaves == 0) {
            ...
            if (initialSPscore < /*0.05*/ opt.sp_score) {
                ...
                createNewRoots(which - 1, opt, max_nodename, max_lineTaxonomy,
                               t);
            } else {
...
void populate(int *nodes, char **taxnames, int node) {
    ...
    for (i = 0; i < 100; i++) {
        // if
        // (strcmp(taxonomyArr[0][treeArr[0][node].taxIndex[0]][treeArr[0][node].taxIndex[1]],taxnames[i])==0){
        if (strcmp(taxonomyArr[0][treeArr[0][node].taxIndex[0]][2],
                   taxnames[i]) == 0) {
...
void parseNewick(struct masterArr *m, const char *newick, int max_nodename) {
    ...
    while (*p) {
        if (*p == '(') {
            ...
            if (current == -1) {
                fprintf(stderr,
                        "Error: Mismatched parentheses in Newick string.\n");
                exit(EXIT_FAILURE);
            }
            p++;
    ...
    if (current != -1) {
        fprintf(stderr,
                "Error: Unmatched opening parenthesis in Newick string.\n");
        exit(EXIT_FAILURE);
    }
}
...
void resolvePolytomy(struct masterArr *m, int nodeIndex, int max_nodename) {
    while (m->tree[nodeIndex].nd > 2) {
        ...
        // printf("Resolving polytomy at node %d: combined children %d and %d
        // into new internal node %d\n",nodeIndex, childIndex1, childIndex2,
        // newNodeIndex);
        ...
        // newNode->nd, newNode->up[0], newNode->up[1], newNode->down);
    }
}
...
int main(int argc, char **argv) {
    ...
    if (opt.number_of_trees == 1 && opt.use_partitions == 0) {
        ...
        specifications[2] = 0;
        gzFile infile;
        if ((infile = gzopen(opt.msa_file, "r")) == NULL)
        ...
        sprintf(m->index, "%d", 0);
        m->tree =
            (struct node *)malloc((2 * m->numspec - 1) * sizeof(struct node));
        ...
        if ((treefile = fopen(opt.tree_file, "r")) == (FILE *)NULL)
            fprintf(stderr, "*** tree file could not be opened.\n");
        ...
        tax_specs[1] = 0;
        FILE *taxfile;
        if ((taxfile = fopen(opt.taxonomy_file, "r")) == (FILE *)NULL)
            fprintf(stderr, "Taxonomy file could not be opened.\n");
        ...
        assignTaxonomyToLeavesArr(opt.taxonomy_file, m, max_nodename,
                                  max_tax_name);
        ...
        pf->tree_files =
            (char **)malloc(sizeof(char *) * opt.number_of_partitions);
        pf->msa_files =
            (char **)malloc(sizeof(char *) * opt.number_of_partitions);
        pf->tax_files =
            (char **)malloc(sizeof(char *) * opt.number_of_partitions);
        ...
        for (i = 0; i < opt.number_of_partitions; i++) {
            FILE *taxfiles;
            snprintf(buffer, BUFFER_SIZE, "%s/%s", opt.readdir,
                     pf->tax_files[i]);
            if (NULL == (taxfiles = fopen(buffer, "r"))) {
                puts("Cannot open tax file!");
                exit(-1);
            }
        ...
        // ***)); nodeIDsArr = (char
        // ***)malloc(opt.number_of_partitions*sizeof(char**)); seqArr = (int
        // ***)malloc(opt.number_of_partitions*sizeof(int **));
        /*for(i=0; i<MAX_NUMBEROFROOTS; i++){
        ...
        for (i = 0; i < partition_count; i++) {
            ...
            gzclose(partition);
            m->numNodes = 0;
            m->tree = (struct node *)malloc((2 * m->numspec - 1) *
                                            sizeof(struct node));
            m->msa = (int **)malloc(m->numspec * sizeof(int *));
            ...
            if ((partitionTree = fopen(buffer, "r")) == (FILE *)NULL)
                printf("*** tree file could not be opened.\n");
            comma = 0;
            tip = 0;
            ...
            assignTaxonomyToLeavesArr(buffer, m, max_nodename,
                                      max_lineTaxonomy);
            getTaxonomyArr(m->root, m);
            hashmap_put(&mastermap, m->index, m);
    ...
    free(SPscoreArr);
    if (opt.two_step == 1) {
        printf("Partitioning has finished and you've requested a two-step "
               "build ('-p') exiting...\n");
        int numberOfTrees = 0;
        int key;
        ...
        if (opt.remove_unused == 1) {
            ...
            while ((de = readdir(dr)) != NULL) {
                ...
                if (!reti) {
                    ...
                    for (index = strlen(tempfilename) - 10;
                         index < strlen(tempfilename); index++) {
                        tempfilename[index] = '\0';
                    }
                    ...
                    snprintf(partition_buffer, BUFFER_SIZE,
                             "%s/RAxML_bestTree.%s.reroot",
                             opt.partitions_directory, tempfilename);
                    printf("%s\n", partition_buffer);
                    int found = 0;
                    ...
                    if (found == 0) {
                        ...
                        snprintf(partition_buffer2, BUFFER_SIZE,
                                 "rm %s/%s_MSA.fasta", opt.partitions_directory,
                                 tempfilename);
                        int status = 0;
                        ...
                        snprintf(partition_buffer2, BUFFER_SIZE,
                                 "rm %s/RAxML_bestTree.%s",
                                 opt.partitions_directory, tempfilename);
                        status = 0;
                        status = system(partition_buffer2);
                        snprintf(partition_buffer2, BUFFER_SIZE,
                                 "rm %s/RAxML_bestTree.%s.reroot",
                                 opt.partitions_directory, tempfilename);
                        status = 0;
                        status = system(partition_buffer2);
                        snprintf(partition_buffer2, BUFFER_SIZE,
                        ...
                        status = system(partition_buffer2);
                        snprintf(partition_buffer2, BUFFER_SIZE,
                                 "rm %s/%s_MSA.phymlAln.reduced",
                                 opt.partitions_directory, tempfilename);
                        status = 0;
                        status = system(partition_buffer2);
                        snprintf(partition_buffer2, BUFFER_SIZE,
                                 "rm %s/RAxML_log.%s", opt.partitions_directory,
                                 tempfilename);
                        status = 0;
                        status = system(partition_buffer2);
                        snprintf(partition_buffer2, BUFFER_SIZE,
        ...
        snprintf(newick_buf, BUFFER_SIZE, "%s/tree_list.txt",
                 opt.partitions_directory);
        FILE *list_newick_files = fopen(newick_buf, "w");
        if (list_newick_files == NULL) {
            printf("Error opening tree_list.txt file!\n");
            exit(1);
        }
        hashmap_foreach(key, final, &mastermap) {
            ...
            if (filename != NULL) {
                ...
                filename = final->filename; // No '/' found, use the whole path
                                            // as filename
            }
            ...
            if (start_substring != NULL) {
                start_substring +=
                    strlen("RAxML_bestTree."); // Move past "RAxML_bestTree."
            ...
            if (last_slash != NULL) {
                ...
                if (strcmp(directory, opt.readdir) == 0) {
                    ...
                    snprintf(command, BUFFER_SIZE,
                             "cp %s/RAxML_bestTree.%s.reroot %s", opt.readdir,
                             substring, opt.partitions_directory);
                    int status = 0;
                    printf("copying %s/RAxML_bestTree.%s.reroot to %s...\n",
                           opt.readdir, substring, opt.partitions_directory);
                    status = system(command);
                    snprintf(command, BUFFER_SIZE, "cp %s/%s_MSA.fasta %s",
                             opt.readdir, substring, opt.partitions_directory);
                    status = 0;
                    printf("copying %s/%s_MSA.fasta to %s...\n", opt.readdir,
                           substring, opt.partitions_directory);
                    status = system(command);
                    snprintf(command, BUFFER_SIZE, "cp %s/%s_taxonomy.txt %s",
                             opt.readdir, substring, opt.partitions_directory);
                    status = 0;
    ...
    int key;
    struct masterArr *final;
    hashmap_foreach(key, final, &mastermap) { numberOfTrees++; }
    ...
    FILE *list_newick_files = fopen(newick_buf, "w");
    if (list_newick_files == NULL) {
        printf("Error opening list.txt file!\n");
        exit(1);
    }
    hashmap_foreach(key, final, &mastermap) {
    ...
    // FILE *for_monica =
    // fopen("/space/s1/lenore/trout_copy/s2_copy/for_monica/OU061397_1_PP.txt","w");
    ...
    printTreeFile(numberOfTrees, max_nodename, max_tax_name, max_lineTaxonomy,
                  opt);
    /*hashmap_foreach(key,final,&mastermap){
