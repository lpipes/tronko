...
/*int lnsrch(int n, double xold[], double fold, double g[], double p[], double
x[], double *f, double stpmax, int *check, double (*func)(double []), double
        ...
        for (;;) {
                for (i=1;i<=n;i++)
                        ...
                        if  (new1 > upbound[i])
                                new1 = upbound[i];
                        x[i] = new1;
                        //printf("x[i]: %f,newin[i]: %f,xold[i]: %f,alam:
                        ...
                        *check=1;
                        ...
                        if (alam == 1.0)
                                ...
                                if (a == 0.0) tmplam = -slope/(2.0*b);
                                        ...
                                        if (disc<0.0) {*/
/*nrerror("Roundoff problem in lnsrch.") return -1;*/ /*disc=0.0;}
                                                       */
...
int lnsrch_Arr(int n, double xold[], double fold, double g[], double p[],
               double x[], double *f, double stpmax, int *check,
               double (*func)(double[], int), double lowbound[],
               double upbound[], int whichRoot) {
    int i;
    ...
    *check = 0;
    ...
    for (;;) {
        for (i = 1; i <= n; i++) {
            ...
            if (new1 < lowbound[i]) /*my bloody code*/
                new1 = lowbound[i];
            if (new1 > upbound[i]) new1 = upbound[i];
            x[i] = new1;
            /*printf("x[i]: %f,newin[i]: %f,xold[i]: %f,alam: %f,p[i]:
             * %f\n",x[i],newin[i],xold[i],alam,p[i]);*/
        }
        *f = (*func)(x, whichRoot);
        if (alam < alamin) {
...
/*void dfpmin(double p[], int n, double gtol, int *iter, double *fret,
        double(*func)(double []), void (*dfunc)(double [], double [],double [],
double [], double(*fu)(double [])), double lowbound[], double upbound[])
{
        int lnsrch(int n, double xold[], double fold, double g[], double p[],
double x[], double *f, double stpmax, int *check, double (*func)(double []),
double lowbound[], double upbound[]); int check,i,its,j; double
den,fac,fad,fae,fp,stpmax,sum=0.0,sumdg,sumxi,temp,test;
        ...
        for (its=1;its<=ITMAX;its++) {
                *iter=its;
                printf ("A: "); for (i=1;i<=n;i++) printf("%f ",g[i]);
printf("\n"); if
(lnsrch(n,p,fp,g,xi,pnew,fret,stpmax,&check,func,lowbound,upbound) == -1) //MY
CODE return;		fp = *fret; for (i=1;i<=n;i++) { xi[i]=pnew[i]-p[i];
                        p[i]=pnew[i];
                }
                test=0.0;
                for (i=1;i<=n;i++) {
                ...
                (*dfunc)(p,g,lowbound,upbound, func);
        //	printf ("C: "); for (i=1;i<=n;i++) printf("%f ",g[i]);
printf("\n"); test=0.0; den=FMAX(*fret,1.0); for (i=1;i<=n;i++) {
...
void dfpmin_Arr(double p[], int n, double gtol, int *iter, double *fret,
                ...
                void (*dfunc)(double[], double[], double[], double[],
                              double (*fu)(double[], int), int),
                double lowbound[], double upbound[], int whichRoot) {
    int lnsrch_Arr(int n, double xold[], double fold, double g[], double p[],
                   double x[], double *f, double stpmax, int *check,
                   double (*func)(double[], int), double lowbound[],
                   double upbound[], int whichRoot);
    ...
    for (its = 1; its <= ITMAX; its++) {
        *iter = its;
        ...
        if (lnsrch_Arr(n, p, fp, g, xi, pnew, fret, stpmax, &check, func,
                       lowbound, upbound, whichRoot) == -1) /*MY CODE*/
            return;
        fp = *fret;
        for (i = 1; i <= n; i++) {
            xi[i] = pnew[i] - p[i];
        ...
        if (fac * fac > EPS * sumdg * sumxi) {
            ...
            for (i = 1; i <= n; i++) {
                for (j = 1; j <= n; j++) {
                    hessin[i][j] +=
                        (double)(fac * xi[i] * xi[j] - fad * hdg[i] * hdg[j] +
                                 fae * dg[i] * dg[j]);
                }
            }
        }
...
/*void Yanggradient (int n, double x[], double f0, double g[],
    double (*fun)(double x[]), double space[], int central, double lowbound[],
double upbound[])
{
...
void Yanggradient_Arr(int n, double x[], double f0, double g[],
                      double (*fun)(double x[], int), double space[],
                      int central, double lowbound[], double upbound[],
                      int whichRoot) {
    ...
    if (central) {
        for (i = 1; i <= n; i++) {
            ...
            if (x0[i] < lowbound[i]) {
                x1[i] += eh;
                g[i] = ((*fun)(x1, whichRoot) - f0) / (eh * 2.0);
            } else if (x1[i] > upbound[i]) {
                x0[i] -= eh;
                g[i] = (f0 - (*fun)(x0, whichRoot)) / (eh * 2.0);
            } else
                g[i] = ((*fun)(x1, whichRoot) - (*fun)(x0, whichRoot)) /
                       (eh * 2.0);
            if (x[i] <= lowbound[i] && g[i] > 0.0) g[i] = 0.0;
            else if (x[i] >= upbound[i] && g[i] < 0.0) g[i] = 0.0;
        }
    } else {
        for (i = 1; i <= n; i++) {
            for (j = 1; j <= n; j++) x1[j] = x[j];
...
/*void getgradient(double invec[], double outvec[], double lowbound[], double
upbound[], double(*func)(double []))
    ...
    if (CENTRALMODE > 0 && PRECISIONLEVEL >1)
                Yanggradient(npar, invec, f0, outvec, func, space, 1, lowbound,
upbound); else Yanggradient(npar, invec, f0, outvec, func, space, 0, lowbound,
upbound);
//	printf("GRADIENT:\n");
...
void getgradient_Arr(double invec[], double outvec[], double lowbound[],
                     double upbound[], double (*func)(double[], int),
                     int whichRoot)

{
    ...
    if (CENTRALMODE > 0 && PRECISIONLEVEL > 1)
        Yanggradient_Arr(npar, invec, f0, outvec, func, space, 1, lowbound,
                         upbound, whichRoot);
    else
        Yanggradient_Arr(npar, invec, f0, outvec, func, space, 0, lowbound,
                         upbound, whichRoot);
    //	printf("GRADIENT:\n");
    /*if (NULL==(tempfile=fopen("tempfile","w"))){
           puts ("Kan ikke aabne tempfile!");
    ...
    if (CENTRALMODE == 0 && fabs(oldf0 - f0) < 0.1) {
        CENTRALMODE = 1;
        /*printf("Changing to central method\n");*/
    } else if (CENTRALMODE > 0 && fabs(oldf0 - f0) > 1.0) CENTRALMODE = 0;
    else if (arbitrarysum < 0.01 * npar) CENTRALMODE = 2;
    oldf0 = f0;
    //	printf(" Like (grad): %f\n",-oldf0);
}

/*Call nrinits before calling this the first time.  Precision level can be 0, 1
 * or 2*/
/*double findmax(double newinvecter[], double lowbound[], double upbound[], int
   n, double (*fun)(double x[]), int precisionlevel)
    ...
    PRECISIONLEVEL = precisionlevel;
        ...
        //do    {
        dfpmin(newinvecter, npar, gtol, &iter, &fret, fun, getgradient,
   lowbound, upbound);
        //}while (CENTRALMODE < 2 && PRECISIONLEVEL == 2);
        return -fret;
        }*/
double findmax_Arr(double newinvecter[], double lowbound[], double upbound[],
                   int n, double (*fun)(double x[], int), int precisionlevel,
                   int whichRoot)

{
    ...
    // do    {
    dfpmin_Arr(newinvecter, npar, gtol, &iter, &fret, fun, getgradient_Arr,
               lowbound, upbound, whichRoot);
    //}while (CENTRALMODE < 2 && PRECISIONLEVEL == 2);
    return (double)-fret;
}
