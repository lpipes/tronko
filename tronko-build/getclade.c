...
int getcladeArr_UsePartitions(FILE *tre, int whichPartitions,
                              char ***nodeIDsArr_heap) {
    int n1, n2, n3;
    char ch;
    n1 = -1;
    n2 = -1;
    n3 = -1;
    do {
        if (specsearchArr_UsePartitions(tre, whichPartitions,
                                        nodeIDsArr_heap) == 1) {
        ...
        if (ch == '(') {
            n3 = getnodenumbArr(tre);
            n1 = getcladeArr_UsePartitions(tre, whichPartitions,
                                           nodeIDsArr_heap);
            n2 = getcladeArr_UsePartitions(tre, whichPartitions,
                                           nodeIDsArr_heap);
            if (n1 != -1 && n2 != -1 && n3 != -1) {
                linknodesArr(n1 - 1, n2 - 1, n3 - 1, whichPartitions);
            }
            if (n1 == -1 || n2 == -1 || n3 == -1) {
                ungetc(ch, tre);
            }
...
int specsearchArr_UsePartitions(FILE *tre, int whichPartitions,
                                char ***nodeIDsArr_heap) {
    char ch;
    int i = 0;
    char specname[30];
    ...
    if ((ch != ')') && (ch != '(') && (ch != ',') && (ch != ' ') &&
        ...
        strcpy(treeArr[whichPartitions][tip + numspecArr[whichPartitions] - 2]
                   .name,
               specname);
        treeArr[whichPartitions][tip + numspecArr[whichPartitions] - 2].up[0] =
            -1;
        treeArr[whichPartitions][tip + numspecArr[whichPartitions] - 2].up[1] =
            -1;
        fscanf(tre, "%lf",
               &treeArr[whichPartitions][tip + numspecArr[whichPartitions] - 2]
                    .bl);
        return 1;
    } else {
        ungetc(ch, tre);
