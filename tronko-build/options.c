...
void parse_options(int argc, char **argv, Options *opt) {
    ...
    while (1) {
        c = getopt_long(argc, argv,
                        "hspvlgaryu:t:m:d:o:x:1:2:i:c:e:n:b:D:f:", long_options,
                        &option_index);
        if (c == -1) break;
        switch (c) {
        case 'h':
        ...
        case 'f':
            ...
            if (!success) fprintf(stderr, "Could not read min leaves\n");
            break;
        case 'p':
            opt->two_step = 1;
            break;
        ...
        case 't':
            ...
            if (!success) fprintf(stderr, "Invalid tree file.\n");
            break;
        case 'i':
            success = sscanf(optarg, "%s", opt->prefix);
            if (!success) fprintf(stderr, "Invalid prefix.\n");
            break;
        case 'm':
        ...
        case 'c':
            ...
            if (!success) fprintf(stderr, "Invalid int\n");
            break;
        case 'e':
