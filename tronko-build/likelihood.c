...

{
  ...
  if (tree[node].up[0]==-1 && tree[node].mrca < 0) {printf("Error in
identication of MRCAs\n"); exit(-1);}*//*this can eventually be removed*/
/*else if (tree[node].up[0]==-1){
  ...
  for (i=0; i<STATESPACE; i++)
    tree[node].like[i] = pdata(statevector[i], tree[node].s,
tree[node].numsites, numbase);*//*here we would also need to change 'numbases to match the number of bases in the alignemnt*/
/* }
 else {
...
void makecon_Arr(int node, int whichRoot) {
    ...
    if (treeArr[whichRoot][node].up[0] == -1 &&
        treeArr[whichRoot][node].mrca < 0) {
        printf("Error in identication of MRCAs\n");
        exit(-1);
    } /*this can eventually be removed*/
    else if (treeArr[whichRoot][node].up[0] == -1) {
        for (i = 0; i < STATESPACE; i++) treeArr[whichRoot][node].like[i] = 1.0;
    } else if (treeArr[whichRoot][node].mrca > -1) {
        for (i = 0; i < STATESPACE; i++)
            treeArr[whichRoot][node].like[i] = pdata(
                statevector[i], treeArr[whichRoot][node].s,
                treeArr[whichRoot][node].numsites,
                numbaseArr[whichRoot]); /*here we would also need to change
                                           'numbases to match the number of
                                           bases in the alignemnt*/
    } else {
        makecon_Arr(treeArr[whichRoot][node].up[0], whichRoot);
        makecon_Arr(treeArr[whichRoot][node].up[1], whichRoot);
        maketransitionmatrix(
            0, treeArr[whichRoot][treeArr[whichRoot][node].up[0]].bl);
        maketransitionmatrix(
            1, treeArr[whichRoot][treeArr[whichRoot][node].up[1]].bl);
        // printf("node %i\n",node);
        max = 0.0;
        for (i = 0; i < STATESPACE; i++) {
            L1 = L2 = 0;
            for (j = 0; j < STATESPACE; j++) {
                L1 +=
                    PMAT1[i][j] *
                    treeArr[whichRoot][treeArr[whichRoot][node].up[0]].like[j];
                L2 +=
                    PMAT2[i][j] *
                    treeArr[whichRoot][treeArr[whichRoot][node].up[1]].like[j];
            }
            // printf("(%lf, %lf) ",L1, L2);
        ...
        // underflow control
        for (i = 0; i < STATESPACE; i++)
            treeArr[whichRoot][node].like[i] =
                treeArr[whichRoot][node].like[i] / max;
        m = log(max);
        UFC = UFC + m;
    }
}
...
{ /*assumes uniform prior distribution on the state space*/
    ...
    if (treeArr[whichRoot][rootArr[whichRoot]].mrca > -1) {
    ...
    maketransitionmatrix(
        0, treeArr[whichRoot][treeArr[whichRoot][rootArr[whichRoot]].up[0]].bl);
    maketransitionmatrix(
        1, treeArr[whichRoot][treeArr[whichRoot][rootArr[whichRoot]].up[1]].bl);
    for (i = 0; i < STATESPACE; i++) {
        L1 = L2 = 0;
        for (j = 0; j < STATESPACE; j++) {
            L1 +=
                PMAT1[i][j] *
                treeArr[whichRoot][treeArr[whichRoot][rootArr[whichRoot]].up[0]]
                    .like[j];
            L2 +=
                PMAT2[i][j] *
                treeArr[whichRoot][treeArr[whichRoot][rootArr[whichRoot]].up[1]]
                    .like[j];
        }
        Lsum += L1 * L2;
    }
    return log(Lsum) +
           UFC; /*the stationary distribution is uniform after scaling so we do
                   not need to multiply with the stationary probabilities*/
}
...
double maximizelikelihood_Arr(double parameters[4], int precision,
                              int whichRoot) {

    // optimization function starts counting at 1 so arrays have dimensionality
    // n+1
    double L, lowbound[4], upbound[4];
    ...
    parameters[1] = parameters[2] = parameters[3] = 1.0;
    lowbound[1] = lowbound[2] = lowbound[3] = 0.000001;
    upbound[1] = upbound[2] = 100.0;
    upbound[3] = 1000.0;
    // L=findmax(parameters, lowbound, upbound, 3, getlikelihood, precision);
    L = findmax_Arr(parameters, lowbound, upbound, 3, getlikelihood_Arr,
                    precision, whichRoot);
    freeNRinits(4);
    return L;
}

// this algorithm runs from the root to calculate the conditional likelihood
// given all data towards the parental node for each node
/*void makeposterior(int node)
{
  ...
  for (i=0; i<STATESPACE; i++){
    ...
    for (j=0; j<STATESPACE; j++)
      tree[node].posterior[i] = tree[node].posterior[i] +
PMAT1[i][j]*templike[j];
  }
...
// for deeper nodes it gives the conditional likelihood given all data towards
// the parental node
...
/* for (i=0; i<STATESPACE; i++)
 ...
 for (j=0; j<2*numspec-1; j++){
   if (tree[j].mrca > -1){
     ...
     {
       if (tree[j].up[0]>-1)
         tree[j].posterior[i] = tree[j].posterior[i]*pdata(statevector[i],
 tree[j].s, tree[j].numsites, numbase); *//*here we would also need to change 'numbases'*/
        ...
        for (i=0; i<4; i++)
  {
    for (j=0; j<4; j++)
    {
      ...
      for (k=0; k<4; k++){
                  ...
                  PMATnc[n][i][j] =  PMATnc[n][i][j] +
RRVECnc[k][j]*LRVECnc[i][k]*EXPOS[k];
...
void maketransitionmatrixnc_Arr(int n, double t, int whichRoot) {
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            ...
            for (k = 0; k < 4; k++) {
                PMATnc[n][i][j] =
                    PMATnc[n][i][j] + RRVECnc[k][j] * LRVECnc[i][k] * EXPOS[k];
            }
        }
    }
}
...

{
  ...
  child = tree[node].up[0];
    ...
    //if (node ==root) printf("node: %i, child: %i, UP0:
  ...
  if (tree[child].up[0]==-1){
    ...
    for (site=0; site<numbase; site++){
      for (i=0; i<4; i++)
       ...
       if (site==0 && node ==root) printf("node 10 (b=%i): %lf %lf %lf
%lf\n",seq[seqn][site],PMATnc[0][0][seq[seqn][site]],PMATnc[0][1][seq[seqn][site]],PMATnc[0][2][seq[seqn][site]],PMATnc[0][3][seq[seqn][site]]);
    ...
    for (site=0; site<numbase; site++)
    {
      for (i=0; i<4; i++){
        ...
        for (j=0; j<4; j++)
          tree[node].likenc[site][i] +=
PMATnc[0][i][j]*tree[child].likenc[site][j];
      }
    }
  }
...
tree[node].likenc[site][i]*PMATnc[0][i][seq[seqn][site]];
    ...
    for (site=0; site<numbase; site++)
    {
      ...
      //might be worth with some code here dealing with the case of
max=0+epsilon
      ...
      //  if (node ==root && site==0) printf("Node %i, Conditional likes: %lf
%lf %lf %lf,
",node,tree[node].likenc[site][0],tree[node].likenc[site][1],tree[node].likenc[site][2],tree[node].likenc[site][3]);
...
void makeconnc_Arr(int node, double lambda, int whichRoot) {
    ...
    if (treeArr[whichRoot][child].up[0] == -1) {
        maketransitionmatrixnc_Arr(0, lambda * treeArr[whichRoot][child].bl,
                                   whichRoot);
        seqn = child - numspecArr[whichRoot] + 1;
        for (site = 0; site < numbaseArr[whichRoot]; site++) {
            for (i = 0; i < 4; i++)
                treeArr[whichRoot][node].likenc[site][i] =
                    PMATnc[0][i][seqArr[whichRoot][seqn][site]];
            if (site == 0 && node == rootArr[whichRoot])
                printf("node 10 (b=%i): %lf %lf %lf %lf\n",
                       seqArr[whichRoot][seqn][site],
                       PMATnc[0][0][seqArr[whichRoot][seqn][site]],
                       PMATnc[0][1][seqArr[whichRoot][seqn][site]],
                       PMATnc[0][2][seqArr[whichRoot][seqn][site]],
                       PMATnc[0][3][seqArr[whichRoot][seqn][site]]);
        }
    } else {
        ...
        for (site = 0; site < numbaseArr[whichRoot]; site++) {
            for (i = 0; i < 4; i++) {
                ...
                for (j = 0; j < 4; j++)
                    treeArr[whichRoot][node].likenc[site][i] +=
                        PMATnc[0][i][j] *
                        treeArr[whichRoot][child].likenc[site][j];
            }
        }
    }
    ...
    if (treeArr[whichRoot][child].up[1] == -1) {
        ...
        for (site = 0; site < numbaseArr[whichRoot]; site++) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                for (j = 0; j < 4; j++)
                    L += PMATnc[0][i][j] *
                         treeArr[whichRoot][child].likenc[site][j];
                // underflow control
                if ((treeArr[whichRoot][node].likenc[site][i] =
                         treeArr[whichRoot][node].likenc[site][i] * L) > max)
            ...
            for (i = 0; i < 4; i++)
                treeArr[whichRoot][node].likenc[site][i] =
                    treeArr[whichRoot][node].likenc[site][i] / max;
            //  if (node ==root && site==0) printf("Node %i, Conditional likes:
            //  %lf %lf %lf %lf,
            //  ",node,tree[node].likenc[site][0],tree[node].likenc[site][1],tree[node].likenc[site][2],tree[node].likenc[site][3]);
            UFCnc[site] = UFCnc[site] + log(max);
            //  printf("max: %lf, UFC: %lf\n", max,UFCnc[site]);
...

{
    ...
    if (eigen(1, A[0], 4, RRVALnc, RIVAL, RRVECnc[0], RIVEC[0], workspace) !=
        0) {
        printf("Transitions matrix did not converge or contained non-real "
               "values!\n");
        exit(-1);
    }
...
double stand, L, loclike, **locloglike, max, pi[4], gampar[2], d, like = 0.0;
  ...
  gampar[0]=gampar[1]=par[9]; //We are setting alpha=beta to keep a constant
mean to avoid identifiability issues.  This is not the same as a standard
gammma.
        //printf("piA: %.2lf, piC: %.2lf, piG: %.2lf, piT: %.2lf, pAC: %.2lf,
",pi[0],pi[1],pi[2],pi[3],par[4]);
        //printf("numbase is %d\n",numbase);
        //printf("pAG: %.2lf, pAT: %.2lf, pCG: %.2lf, pCT: %.2lf, pGT: 1.00, a:
%.2lf ",par[5],par[6],par[7],par[8],par[9]);
...
double getlike_gamma_Arr(double par[10], int whichRoot) {
    ...
    double stand, L, loclike, **locloglike, max, pi[4], gampar[2], d,
        like = 0.0;
    int i, j, k;
    COUNT2++;
    stand = 1.0 + par[1] + par[2] + par[3];
    ...
    pi[3] = 1.0 - pi[0] - pi[1] - pi[2];
    gampar[0] = gampar[1] =
        par[9]; // We are setting alpha=beta to keep a constant mean to avoid
                // identifiability issues.  This is not the same as a standard
                // gammma. printf("piA: %.2lf, piC: %.2lf, piG: %.2lf, piT:
                // %.2lf, pAC: %.2lf, ",pi[0],pi[1],pi[2],pi[3],par[4]);
                // printf("numbase is %d\n",numbase);
                // printf("pAG: %.2lf, pAT: %.2lf, pCG: %.2lf, pCT: %.2lf,
                // pGT: 1.00, a: %.2lf ",par[5],par[6],par[7],par[8],par[9]);
    ...
    inittransitionmatrixnc(pi, par);
    for (j = 0; j < NUMCAT; j++) {
        for (i = 0; i < numbaseArr[whichRoot]; i++) UFCnc[i] = 0.0;
        makeconnc_Arr(rootArr[whichRoot], statevector[j], whichRoot);
        for (i = 0; i < numbaseArr[whichRoot]; i++) {
            L = 0.0;
            for (k = 0; k < 4; k++)
                L +=
                    treeArr[whichRoot][rootArr[whichRoot]].likenc[i][k] * pi[k];
            if (L > 0.0) locloglike[i][j] = log(L) + UFCnc[i];
        }
    }
    for (i = 0; i < numbaseArr[whichRoot]; i++) {
        ...
        for (j = 0; j < NUMCAT; j++) {
            d = locloglike[i][j] - max;
            if (d > -100) loclike += exp(d);
        }
        like = like + log(loclike) + max;
    }
    free(statevector);
    for (i = 0; i < numbaseArr[whichRoot]; i++) free(locloglike[i]);
    free(locloglike);
    // printf("LIKE: %lf\n",like - (double)numbase*log((double)NUMCAT));
    // printf("\n");
    free(UFCnc);
...
double maximizelikelihoodnc_globals_Arr(double parameters[10], int precision,
                                        int whichRoot) {
    // optimization function starts counting at 1 so arrays have dimensionality
    // n+1
    int i;
    ...
    lowbound[9] = 0.3;
    // L=findmax(parameters, lowbound, upbound, 10, getlike_gamma, precision);
    L = (double)findmax_Arr(parameters, lowbound, upbound, 9, getlike_gamma_Arr,
                            precision, whichRoot);
    freeNRinits(10);
    return L;
}
...
p,localpi[j],PMATnc[0][base][j],templike_nc[s][j]);
        }
      }
      else p=1.0;
        ...
        if (treeArr[whichRoot][localnode].up[0] == -1) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                for (j = 0; j < 4; j++)
                    b += PMATnc[0][i][j] * templike_nc[s][j];
                p +=
                    b * localpi[i] * treeArr[whichRoot][localnode].likenc[s][i];
            }
        }
        L += log(p);
        // printf("L is %lf\n",L);
...
getlike_gamma(currentestimate),tree[node].bl); L = findmax(par, minpar, maxpar,
1, like_bl, precision);
  //printf("Node %i likelihood after: %lf (%lf) ",node,
getlike_gamma(currentestimate),tree[node].bl);
 //printf("Node %i LIKE BL: %lf\n",node, L);
  tree[node].bl = par[1];

}*/
void maxbl_nc_Arr(int node, int parent, double pi[4], int precision,
                  int whichRoot) {
    double par[2], minpar[2], maxpar[2], L;
    par[1] = treeArr[whichRoot][node]
                 .bl; /*This stuff should probably be cleaned up*/
    minpar[1] = MINBL;
    maxpar[1] = MAXBL;
    localpi = pi;
    localnode = node;
  ...
  for (s=0; s<numbase; s++){
    for (i=0; i<4; i++){
      ...
      if (tree[otherb].up[0]==-1){
        templike_nc[s][i] = PMATnc[1][i][seq[otherb-numspec+1][s]]; //if(s==0)
printf("temp[s][%i]: %lf (b=%i, %lf),
",i,templike_nc[s][i],seq[otherb-numspec+1][s],
PMATnc[1][i][seq[otherb-numspec+1][s]]);
                }
      else {
        templike_nc[s][i]=0.0;
        for (j=0; j<4; j++)
      ...
      if
((templike_nc[s][i]=templike_nc[s][i]*tree[parent].posteriornc[s][i])>max)
        max=templike_nc[s][i];
      //      if(s==0 && node ==0) {printf("node %i (%i): temp[s][%i]: %lf,
other/parent: %i/%i like: %lf, parentlike: %lf  ",node,s,i,templike_nc[s][i],
otherb, parent,tree[otherb].likenc[s][i],tree[parent].posteriornc[s][i]);}
    }
  ...
  for (s=0; s<numbase; s++){
    ...
    for (i=0; i<4; i++){
      ...
      for (j=0; j<4; j++)
        tree[node].posteriornc[s][i] = tree[node].posteriornc[s][i] +
PMATnc[0][i][j]*templike_nc[s][j]; if (tree[node].posteriornc[s][i]>max)//more
hysterical underflow protection max=tree[node].posteriornc[s][i];
    }
    for (i=0; i<4; i++)
...
void recurse_estimatebranchlengths_Arr(int node, double pi[4], int precision,
                                       int whichRoot) {
    int i, j, s, parent, otherb, child1, child2;
    double max, bl;
    ...
    for (s = 0; s < numbaseArr[whichRoot]; s++) {
        for (i = 0; i < 4; i++) {
            ...
            if (treeArr[whichRoot][otherb].up[0] == -1) {
                templike_nc[s][i] = PMATnc
                    [1][i]
                    [seqArr[whichRoot][otherb - numspecArr[whichRoot] + 1]
                           [s]]; /*if(s==0) printf("temp[s][%i]: %lf (b=%i,
                                    %lf),
                                    ",i,templike_nc[s][i],seq[otherb-numspec+1][s],
                                    PMATnc[1][i][seq[otherb-numspec+1][s]]);*/
            } else {
                ...
                for (j = 0; j < 4; j++)
                    templike_nc[s][i] =
                        templike_nc[s][i] +
                        treeArr[whichRoot][otherb].likenc[s][j] *
                            PMATnc[1][i][j];
            }
            if ((templike_nc[s][i] =
                     templike_nc[s][i] *
                     treeArr[whichRoot][parent].posteriornc[s][i]) > max)
                max = templike_nc[s][i];
            //      if(s==0 && node ==0) {printf("node %i (%i): temp[s][%i]:
            //      %lf, other/parent: %i/%i like: %lf, parentlike: %lf
            //      ",node,s,i,templike_nc[s][i], otherb,
            //      parent,tree[otherb].likenc[s][i],tree[parent].posteriornc[s][i]);}
        }
        // for (i=0; i<4; i++)//underflow protection
        // templike_nc[s][i]=templike_nc[s][i]/max;
    }
    ...
    for (s = 0; s < numbaseArr[whichRoot]; s++) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            for (j = 0; j < 4; j++)
                treeArr[whichRoot][node].posteriornc[s][i] =
                    treeArr[whichRoot][node].posteriornc[s][i] +
                    PMATnc[0][i][j] * templike_nc[s][j];
            if (treeArr[whichRoot][node].posteriornc[s][i] >
                max) // more hysterical underflow protection
                max = treeArr[whichRoot][node].posteriornc[s][i];
        }
        for (i = 0; i < 4; i++)
            treeArr[whichRoot][node].posteriornc[s][i] =
                treeArr[whichRoot][node].posteriornc[s][i] / max;
    }
    if (child1 > -1) {
...
/*void estimatebracnhlengths(double par[10], int precision)
{
  ...
  //Root is different because only one parameter can be estimated for the two
child nodes. We arbitrarily fix one of the branchlengths to equal MINBL and only
estimate the other one

  if ((tree[child2].bl=tree[child2].bl+tree[child1].bl-MINBL)<MINBL)
    tree[child2].bl=MINBL;
  tree[child1].bl=MINBL;
  getlike_gamma(par); //this is not necessary if the likelihood fucntion has
already been called



  //  printf("Root likelihoods: %lf %lf %lf
%lf\n",tree[root].likenc[s][0],tree[root].likenc[s][1],tree[root].likenc[s][2],tree[root].likenc[s][3]);
  //  printf("Child1 likelihoods: %lf %lf %lf
%lf\n",tree[child1].likenc[s][0],tree[child1].likenc[s][1],tree[child1].likenc[s][2],tree[child1].likenc[s][3]);
  //  printf("Child2 likelihoods: %lf %lf %lf
    ...
    for (i=0; i<4; i++){
      tree[root].posteriornc[s][i] = 1.0;//tree[child1].likenc[s][i];
        if (tree[child2].up[0]>-1){
            ...
            for (j=0; j<4; j++)
                tree[child1].posteriornc[s][i] +=
tree[child2].likenc[s][j]*PMATnc[0][i][j];
            }
        else
                tree[child1].posteriornc[s][i] =
PMATnc[0][i][seq[child2-numspec+1][s]];
            }
    //  if(s==0){
    //    printf("node %i initialization: %lf %lf %lf
%lf\n",root,tree[root].posteriornc[s][0],tree[root].posteriornc[s][1],tree[root].posteriornc[s][2],tree[root].posteriornc[s][3]);
    //   printf("Initial child 1 like: %lf %lf %lf
%lf\n",tree[child1].likenc[s][0],tree[child1].likenc[s][1],tree[child1].likenc[s][2],tree[child1].likenc[s][3]);
    // }
  }
...
void estimatebranchlengths_Arr(double par[10], int precision, int whichRoot) {
    ...
    /*Root is different because only one parameter can be estimated for the two
     * child nodes. We arbitrarily fix one of the branchlengths to equal MINBL
     * and only estimate the other one*/

    if ((treeArr[whichRoot][child2].bl = treeArr[whichRoot][child2].bl +
                                         treeArr[whichRoot][child1].bl -
                                         MINBL) < MINBL)
        treeArr[whichRoot][child2].bl = MINBL;
    treeArr[whichRoot][child1].bl = MINBL;
    getlike_gamma_Arr(par, whichRoot); /*this is not necessary if the likelihood
                                          fucntion has already been called*/
    maketransitionmatrixnc_Arr(
        0, treeArr[whichRoot][child2].bl + treeArr[whichRoot][child1].bl,
        whichRoot);
    for (s = 0; s < numbaseArr[whichRoot]; s++) {
        for (i = 0; i < 4; i++) {
            treeArr[whichRoot][rootArr[whichRoot]].posteriornc[s][i] =
                1.0; /*tree[child1].likenc[s][i];*/
            if (treeArr[whichRoot][child2].up[0] > -1) {
                treeArr[whichRoot][child1].posteriornc[s][i] = 0.0;
                for (j = 0; j < 4; j++)
                    treeArr[whichRoot][child1].posteriornc[s][i] +=
                        treeArr[whichRoot][child2].likenc[s][j] *
                        PMATnc[0][i][j];
            } else
                treeArr[whichRoot][child1].posteriornc[s][i] =
                    PMATnc[0][i][seqArr[whichRoot]
                                       [child2 - numspecArr[whichRoot] + 1][s]];
        }
        //  if(s==0){
        //    printf("node %i initialization: %lf %lf %lf
        //    %lf\n",root,tree[root].posteriornc[s][0],tree[root].posteriornc[s][1],tree[root].posteriornc[s][2],tree[root].posteriornc[s][3]);
    ...
    if (treeArr[whichRoot][child1].up[0] > -1)
        recurse_estimatebranchlengths_Arr(treeArr[whichRoot][child1].up[0], pi,
                                          precision, whichRoot);
    if (treeArr[whichRoot][child1].up[1] > -1)
        recurse_estimatebranchlengths_Arr(treeArr[whichRoot][child1].up[1], pi,
                                          precision, whichRoot);
    recurse_estimatebranchlengths_Arr(child2, pi, precision, whichRoot);
    for (i = 0; i < numbaseArr[whichRoot]; i++) free(templike_nc[i]);
    free(templike_nc);
    freeNRinits(1);
}

// Other schemes could be used for iterative optimizsiton.  This one seems to
// provide a reasobale compromise between time and accuracy
...
void makeposterior_nc(int node)
{
  ...
  for (s=0; s<numbase; s++){
    ...
    for (i=0; i<4; i++){
      ...
      max=0.0;
      for (j=0; j<4; j++){
        if ((tree[node].posteriornc[s][i] = tree[node].posteriornc[s][i] +
PMATnc[0][i][j]*templike_nc[s][j])>max) max=tree[node].posteriornc[s][i];//more
underflow protection
  ...
  //printf("In make posterior node %i: (pa=%lf,pt=%lf) (templikea&t: %lf
    ...
    for (s = 0; s < numbaseArr[whichRoot]; s++) {
        if (treeArr[whichRoot][otherb].up[0] > -1) {
            for (i = 0; i < 4; i++) {
                ...
                templike_nc[s][i] =
                    templike_nc[s][i] *
                    treeArr[whichRoot][parent].posteriornc[s][i];
            }
        ...
        for (i = 0; i < 4; i++) {
            ...
            for (j = 0; j < 4; j++) {
                if ((treeArr[whichRoot][node].posteriornc[s][i] =
                         treeArr[whichRoot][node].posteriornc[s][i] +
                         PMATnc[0][i][j] * templike_nc[s][j]) > max)
                    max = treeArr[whichRoot][node]
                              .posteriornc[s][i]; // more underflow protection
            }
        }
        for (i = 0; i < 4; i++)
            treeArr[whichRoot][node].posteriornc[s][i] =
...
void getposterior_nc(double par[10])
{
  ...
  for (j=0; j<2*numspec-1; j++){
    if (tree[j].up[0]>-1){
      //printf("Node %i: L1=(%lf, %lf, %lf, %lf), alike2=%lf,
",j,tree[j].likenc[0][0],tree[j].likenc[0][1],tree[j].likenc[0][2],tree[j].likenc[0][3],tree[j].posteriornc[0][0]);
      for (s=0; s<numbase; s++) {
        sum = 0.0;
        for (i=0; i<4; i++)
          sum = sum +
(tree[j].posteriornc[s][i]=tree[j].likenc[s][i]*tree[j].posteriornc[s][i]*pi[i]);
        for (i=0; i<4; i++)
     ...
     // printf("posterior(a)=%lf\n",tree[j].posteriornc[0][0]);
      ...
      for (s=0; s<numbase; s++) {
        ...
        b=seq[j-numspec+1][s];
        if (b==4){
          if (notdonebefore==1) {maketransitionmatrixnc(0, tree[j].bl);
notdonebefore=0;} parent=tree[j].down; sum = 0.0; for (i=0; i<4; i++){
            tree[j].posteriornc[s][i]=0.0;
            for (k=0; k<4; k++)
              tree[j].posteriornc[s][i] +=
tree[parent].posteriornc[s][k]*PMATnc[0][i][k]; sum = sum +
(tree[j].posteriornc[s][i]=tree[j].posteriornc[s][i]*pi[i]);
          }
          for (i=0; i<4; i++)
            tree[j].posteriornc[s][i] = tree[j].posteriornc[s][i]/sum;
        }
...
void set_posteriors(int whichRoot) {
    ...
    for (i = 0; i < 2 * numspecArr[whichRoot] - 1; i++) {
        for (j = 0; j < numbaseArr[whichRoot]; j++) {
            for (k = 0; k < 4; k++) {
                treeArr[whichRoot][i].posteriornc[j][k] =
                    log(0.01 / 4 +
                        (1 - 0.01) * treeArr[whichRoot][i].posteriornc[j][k]);
            }
        }
    }
}
...
void getposterior_nc_Arr(double par[10], int whichRoot) {
    ...
    if (treeArr[whichRoot][treeArr[whichRoot][rootArr[whichRoot]].up[0]].up[0] >
        -1)
        makeposterior_nc_Arr(treeArr[whichRoot][rootArr[whichRoot]].up[0],
                             whichRoot);
    if (treeArr[whichRoot][treeArr[whichRoot][rootArr[whichRoot]].up[1]].up[0] >
        -1)
        makeposterior_nc_Arr(treeArr[whichRoot][rootArr[whichRoot]].up[1],
                             whichRoot);
    for (j = 0; j < 2 * numspecArr[whichRoot] - 1; j++) {
        if (treeArr[whichRoot][j].up[0] > -1) {
            ...
            for (s = 0; s < numbaseArr[whichRoot]; s++) {
                ...
                for (i = 0; i < 4; i++)
                    sum = sum +
                          (treeArr[whichRoot][j].posteriornc[s][i] =
                               treeArr[whichRoot][j].likenc[s][i] *
                               treeArr[whichRoot][j].posteriornc[s][i] * pi[i]);
                for (i = 0; i < 4; i++)
                    treeArr[whichRoot][j].posteriornc[s][i] =
                        treeArr[whichRoot][j].posteriornc[s][i] / sum;
            }
            // printf("posterior(a)=%lf\n",tree[j].posteriornc[0][0]);
        } else { // printf("B\n");
            for (s = 0; s < numbaseArr[whichRoot]; s++) {
                ...
                if (b == 4) {
                    if (notdonebefore == 1) {
                        maketransitionmatrixnc_Arr(0, treeArr[whichRoot][j].bl,
                                                   whichRoot);
                        notdonebefore = 0;
                    }
                    ...
                    for (i = 0; i < 4; i++) {
                        ...
                        for (k = 0; k < 4; k++)
                            treeArr[whichRoot][j].posteriornc[s][i] +=
                                treeArr[whichRoot][parent].posteriornc[s][k] *
                                PMATnc[0][i][k];
                        sum =
                            sum + (treeArr[whichRoot][j].posteriornc[s][i] =
                                       treeArr[whichRoot][j].posteriornc[s][i] *
                                       pi[i]);
                    }
                    for (i = 0; i < 4; i++)
